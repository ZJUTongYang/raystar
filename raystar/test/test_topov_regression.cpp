...
static const BoundaryEndpoint* findEndpoint(
  const VisibilityRegion& region, double x, double y, double tolerance = 1e-6)
{
...
TEST(TopoVRegression, LocateVertexCorrect)
{
  ...
  EXPECT_EQ(v1.second, 2)
    << "locateVertex(10,7) should return (0,2) since obs[0][2]=(10,7)";

  auto v2 = pm.locateVertex(9, 7);
  EXPECT_EQ(v2.first, 0);
  EXPECT_EQ(v2.second, 1)
    << "locateVertex(9,7) should return (0,1) since obs[0][1]=(9,7)";
}
...
TEST(TopoVRegression, VisibilityEndpointMetadataMatchesGeometry)
{
  ...
  const auto expect_edge = [&](const BoundaryEndpoint* endpoint,
      std::pair<int, int> expected_from, std::pair<int, int> expected_to) {
  ...
  expect_edge(findEndpoint(from_root, 14.199273, 41.924894, 1e-5),
    {1, 40}, {49, 47});
  ...
  const auto* horizontal_edge =
    std::get_if<DirectedObstacleEdge>(&integer_edge_point->support);
  ...
  expect_edge(findEndpoint(from_obstacle_vertex, 30.0, 9.857142857, 1e-6),
    {30, 16}, {30, 7});

  std::vector<std::pair<double, double>> legacy_positions;
  std::vector<std::pair<int, int>> legacy_topology;
  ASSERT_TRUE(pm.getVisibilityRegion(
    10, 7, legacy_positions, legacy_topology, &error)) << error;
  ASSERT_EQ(legacy_positions.size(), from_obstacle_vertex.size());
  ASSERT_EQ(legacy_topology.size(), from_obstacle_vertex.size());
  const auto integer_edge_position = std::find(
    legacy_positions.begin(), legacy_positions.end(), std::make_pair(17.0, 14.0));
  ASSERT_NE(integer_edge_position, legacy_positions.end());
  const size_t integer_edge_index = static_cast<size_t>(
    std::distance(legacy_positions.begin(), integer_edge_position));
...
TEST(TopoVRegression, Node0TopoVMatchesLocateVertex)
{
  ...
  for (size_t i = 0; i < nodes[0].visibility().size(); ++i)
  {
    ...
    bool is_integer = (nodes[0].visibility()[i].first == ix) &&
      (nodes[0].visibility()[i].second == iy);
    if (!is_integer || nodes[0].visibilityTopology()[i].first < 0) continue;
        ...
        lv.second != nodes[0].visibilityTopology()[i].second)
    {
      ADD_FAILURE() << "V_[" << i << "]=(" << ix << "," << iy << ") topo=("
                    ...
                    << nodes[0].visibilityTopology()[i].second
                    << ") but locateVertex=(" << lv.first << "," << lv.second << ")";
      mismatch_count++;
    }
  }
...
TEST(TopoVRegression, Node1EndAngleReasonable)
{
  ...
  if (angle_span < 0) angle_span += kTwoPi;

  EXPECT_LT(angle_span, kPi)
    << "Node 1 angle span should be < pi (180deg) for a gap, got "
...
TEST(TopoVRegression, Node1FullVisibilityGeometry)
{
  ...
  for (const auto& endpoint : nodes[1].fullVisibilityRegion())
  {
    EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
      source, start, end, exactPoint(endpoint)))
      << "Visibility vertex (" << endpoint.position.first << ","
      << endpoint.position.second << ") lies inside the source obstacle's blocked wedge";
  }
}
...
TEST(TopoVRegression, ScopedVisibilityPreservesBoundarySupport)
{
  ...
  const auto* scoped_fractional = findEndpoint(
    node->visibilityRegion(), 15.0, 10.125);
  const auto* parent_fractional = findEndpoint(
    core.getNodes().front().visibilityRegion(), 15.0, 10.125);
  ...
  EXPECT_EQ(result.polymap->locateVertex(13, 14),
    std::make_pair(exact->obstacle, exact->vertex));
}
...
TEST(TopoVRegression, ExactSameRayOrderingAvoidsSpuriousWideScope)
{
  ...
  {
    if (candidate.seed() != std::make_pair(15, 16) ||
        candidate.parentIndex() < 0 ||
        candidate.parentIndex() >= static_cast<int>(nodes.size()) ||
        nodes[static_cast<size_t>(candidate.parentIndex())].seed() !=
          std::make_pair(10, 7))
    {
      continue;
    }
  ...
  EXPECT_TRUE(exact_geometry::isSameDirectedRay(
    source, exactPoint(*near), exactPoint(*far)));
  ...
  const auto child = std::find_if(
    node->children().begin(), node->children().end(), [&](const Child& candidate) {
...
TEST(TopoVRegression, ChildCtopoMatchesLocateVertex)
{
  ...
  for (size_t ni = 0; ni < nodes.size(); ++ni)
  {
    for (size_t ci = 0; ci < nodes[ni].children().size(); ++ci)
    {
      ...
      EXPECT_TRUE(pm.validateBoundaryEndpoint(c.endpoint(), &endpoint_error))
        << endpoint_error;
      ...
      EXPECT_FALSE(std::holds_alternative<std::monostate>(
        c.oppositeEndpoint().support));

      const bool is_fractional =
        std::abs(c.oppositeEndpoint().position.first -
