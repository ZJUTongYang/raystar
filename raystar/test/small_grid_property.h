...
public:
  ...
  std::size_t bounded(std::size_t upper_exclusive) noexcept {
    return upper_exclusive == 0 ? 0 : static_cast<std::size_t>(next() % upper_exclusive);
  }
...
inline std::uint64_t propertySeed(std::size_t case_index) noexcept {
  return kPropertyBaseSeed + kSeedStride * static_cast<std::uint64_t>(case_index + 1);
}

inline bool isForcedOuterRing(const nav_msgs::msg::OccupancyGrid& map,
                              std::uint32_t x,
                              std::uint32_t y) noexcept {
  return x == 0 || y == 0 || x + 1 == map.info.width || y + 1 == map.info.height;
}

inline bool hasValidOccupancyValues(const nav_msgs::msg::OccupancyGrid& map) noexcept {
  return std::all_of(map.data.begin(), map.data.end(), [](std::int8_t raw_value) {
    const int value = static_cast<int>(raw_value);
    return value == -1 || (value >= 0 && value <= 100);
  });
}

inline bool isEffectivelyBlocked(const nav_msgs::msg::OccupancyGrid& map,
                                 std::uint32_t x,
                                 std::uint32_t y,
                                 bool allow_unknown,
                                 int occupied_threshold = kOccupiedThreshold) noexcept {
  if (x >= map.info.width || y >= map.info.height || isForcedOuterRing(map, x, y)) {
    return true;
  }
...
inline bool fourNeighborReachable(const SmallGridCase& property_case,
  ...
  if (map.info.width == 0 || map.info.height == 0 ||
      ...
      property_case.start.x >= map.info.width || property_case.start.y >= map.info.height ||
      property_case.goal.x >= map.info.width || property_case.goal.y >= map.info.height ||
      occupied_threshold < 1 || occupied_threshold > 100 || !hasValidOccupancyValues(map)) {
    return false;
  }
  if (isEffectivelyBlocked(map,
                           property_case.start.x,
                           property_case.start.y,
                           property_case.allow_unknown,
                           occupied_threshold) ||
      isEffectivelyBlocked(map,
                           property_case.goal.x,
  ...
  const std::size_t cell_count = static_cast<std::size_t>(map.info.width) * map.info.height;
  ...
  for (std::size_t head = 0; head < frontier.size(); ++head) {
    ...
    for (std::size_t direction = 0; direction < dx.size(); ++direction) {
      ...
      if (next_x < 0 || next_y < 0 || next_x >= static_cast<int>(map.info.width) ||
          next_y >= static_cast<int>(map.info.height)) {
        continue;
      }
      const GridCell next{static_cast<std::uint32_t>(next_x), static_cast<std::uint32_t>(next_y)};
      const std::size_t next_index = flat_index(next);
      if (visited[next_index] ||
          isEffectivelyBlocked(
            map, next.x, next.y, property_case.allow_unknown, occupied_threshold)) {
        continue;
      }
...
inline bool hasUnsupportedDiagonalContact(const SmallGridCase& property_case,
                                          int occupied_threshold = kOccupiedThreshold) {
  const auto& map = property_case.map;
  ...
  for (std::uint32_t y = 0; y + 1 < map.info.height; ++y) {
    for (std::uint32_t x = 0; x + 1 < map.info.width; ++x) {
      const bool lower_left =
        isEffectivelyBlocked(map, x, y, property_case.allow_unknown, occupied_threshold);
      const bool lower_right =
        isEffectivelyBlocked(map, x + 1, y, property_case.allow_unknown, occupied_threshold);
      const bool upper_left =
        isEffectivelyBlocked(map, x, y + 1, property_case.allow_unknown, occupied_threshold);
      const bool upper_right =
        isEffectivelyBlocked(map, x + 1, y + 1, property_case.allow_unknown, occupied_threshold);
...
inline SmallGridCase makeSmallGridCase(std::size_t case_index) {
  ...
  map.info.origin.position.x = 0.25 * (static_cast<int>(rng.bounded(17)) - 8);
  map.info.origin.position.y = 0.25 * (static_cast<int>(rng.bounded(17)) - 8);
  ...
  if (!property_case.expected_reachable) {
    ...
    if (vertical) {
      const std::uint32_t barrier_x =
        3 + static_cast<std::uint32_t>(rng.bounded(map.info.width - 6));
      for (std::uint32_t y = 0; y < map.info.height; ++y) blocked[mask_index(barrier_x, y)] = 1;
      property_case.start =
        GridCell{1 + static_cast<std::uint32_t>(rng.bounded(barrier_x - 1)),
                 1 + static_cast<std::uint32_t>(rng.bounded(map.info.height - 2))};
      property_case.goal = GridCell{
        barrier_x + 1 + static_cast<std::uint32_t>(rng.bounded(map.info.width - barrier_x - 2)),
        1 + static_cast<std::uint32_t>(rng.bounded(map.info.height - 2))};
    } else {
      const std::uint32_t barrier_y =
        3 + static_cast<std::uint32_t>(rng.bounded(map.info.height - 6));
      for (std::uint32_t x = 0; x < map.info.width; ++x) blocked[mask_index(x, barrier_y)] = 1;
      property_case.start =
        GridCell{1 + static_cast<std::uint32_t>(rng.bounded(map.info.width - 2)),
                 1 + static_cast<std::uint32_t>(rng.bounded(barrier_y - 1))};
      property_case.goal = GridCell{
        1 + static_cast<std::uint32_t>(rng.bounded(map.info.width - 2)),
        barrier_y + 1 + static_cast<std::uint32_t>(rng.bounded(map.info.height - barrier_y - 2))};
    }
  } else {
    ...
    for (std::size_t attempt = 0; attempt < 64 && accepted_rectangles < target_rectangles;
         ++attempt) {
      const std::uint32_t left = 2 + static_cast<std::uint32_t>(rng.bounded(map.info.width - 4));
      const std::uint32_t bottom = 2 + static_cast<std::uint32_t>(rng.bounded(map.info.height - 4));
      const std::uint32_t right = std::min<std::uint32_t>(
        map.info.width - 3, left + static_cast<std::uint32_t>(rng.bounded(3)));
      const std::uint32_t top = std::min<std::uint32_t>(
        map.info.height - 3, bottom + static_cast<std::uint32_t>(rng.bounded(3)));
      ...
      for (std::uint32_t y = bottom; y <= top; ++y)
        for (std::uint32_t x = left; x <= right; ++x) blocked[mask_index(x, y)] = 1;
      ++accepted_rectangles;
    }
    ...
    if (free_cells.size() < 2) {
      ...
      free_cells = {GridCell{1, 1}, GridCell{map.info.width - 2, map.info.height - 2}};
    }
    const std::size_t start_index = rng.bounded(free_cells.size());
    const std::size_t goal_index =
      (start_index + 1 + rng.bounded(free_cells.size() - 1)) % free_cells.size();
...
inline std::string describeSmallGridCase(const SmallGridCase& property_case) {
  ...
  stream << "T-04 case=" << property_case.case_index << " seed=0x" << std::hex << std::uppercase
         << property_case.seed << std::dec << " size=" << map.info.width << 'x' << map.info.height
         << " resolution=" << map.info.resolution << " origin=(" << map.info.origin.position.x
         << ',' << map.info.origin.position.y << ')' << " allow_unknown=" << std::boolalpha
         << property_case.allow_unknown
         << " expected_reachable=" << property_case.expected_reachable << " start=("
         << property_case.start.x << ',' << property_case.start.y << ") goal=("
         << property_case.goal.x << ',' << property_case.goal.y << ")\n"
  ...
  for (std::uint32_t row = map.info.height; row > 0; --row) {
    ...
    for (std::uint32_t x = 0; x < map.info.width; ++x) {
      ...
      if (GridCell{x, y} == property_case.start)
        ...
        const int value =
          static_cast<int>(map.data[static_cast<std::size_t>(y) * map.info.width + x]);
        if (value == -1)
          symbol = '?';
