...
struct ValidationResult {
  static constexpr std::size_t kNoSegment = std::numeric_limits<std::size_t>::max();
...
inline ExactDyadic subtractExact(const ExactDyadic& lhs, const ExactDyadic& rhs) {
  ...
  return ExactDyadic{
    (lhs.significand << (lhs.exponent - exponent)) - (rhs.significand << (rhs.exponent - exponent)),
    exponent};
}

inline ExactDyadic multiplyExact(const ExactDyadic& lhs, const ExactDyadic& rhs) {
...
inline bool pointsEqual(const ExactPlanarPoint& lhs, const ExactPlanarPoint& rhs) {
  return lhs.x == rhs.x && lhs.y == rhs.y;
}

inline int exactOrientation(const ExactPlanarPoint& first,
                            const ExactPlanarPoint& second,
  ...
  return exactSign(
    subtractExact(multiplyExact(first_x, second_y), multiplyExact(first_y, second_x)));
}

inline bool onClosedSegment(const ExactPlanarPoint& first,
                            const ExactPlanarPoint& second,
...
inline bool segmentsShareEndpoint(const ExactPlanarPoint& first_start,
                                  const ExactPlanarPoint& first_end,
                                  const ExactPlanarPoint& second_start,
                                  const ExactPlanarPoint& second_end) {
  return pointsEqual(first_start, second_start) || pointsEqual(first_start, second_end) ||
         pointsEqual(first_end, second_start) || pointsEqual(first_end, second_end);
}

inline SelfIntersectionKind classifyClosedSegmentIntersection(const ExactPlanarPoint& first_start,
                                                              const ExactPlanarPoint& first_end,
                                                              const ExactPlanarPoint& second_start,
                                                              const ExactPlanarPoint& second_end) {
  const int first_first = exactOrientation(first_start, first_end, second_start);
  const int first_second = exactOrientation(first_start, first_end, second_end);
  const int second_first = exactOrientation(second_start, second_end, first_start);
  const int second_second = exactOrientation(second_start, second_end, first_end);

  if (first_first == 0 && first_second == 0 && second_first == 0 && second_second == 0) {
    const bool use_x = first_start.x != first_end.x || second_start.x != second_end.x;
    const double first_lower =
      use_x ? std::min(first_start.x, first_end.x) : std::min(first_start.y, first_end.y);
    const double first_upper =
      use_x ? std::max(first_start.x, first_end.x) : std::max(first_start.y, first_end.y);
    const double second_lower =
      use_x ? std::min(second_start.x, second_end.x) : std::min(second_start.y, second_end.y);
    const double second_upper =
      use_x ? std::max(second_start.x, second_end.x) : std::max(second_start.y, second_end.y);
    ...
    return segmentsShareEndpoint(first_start, first_end, second_start, second_end)
             ? SelfIntersectionKind::kRepeatedVertex
             : SelfIntersectionKind::kNonAdjacentTouch;
  }

  if (first_first * first_second < 0 && second_first * second_second < 0) {
    return SelfIntersectionKind::kProperCrossing;
  }

  const bool touches =
    (first_first == 0 && onClosedSegment(first_start, first_end, second_start)) ||
    (first_second == 0 && onClosedSegment(first_start, first_end, second_end)) ||
    (second_first == 0 && onClosedSegment(second_start, second_end, first_start)) ||
    (second_second == 0 && onClosedSegment(second_start, second_end, first_end));
  if (!touches)
    return SelfIntersectionKind::kNone;
  return segmentsShareEndpoint(first_start, first_end, second_start, second_end)
           ? SelfIntersectionKind::kRepeatedVertex
...
inline SelfIntersectionResult selfIntersectionInconclusive(std::string diagnostic) {
...
inline std::string formatGridPoint(const GridPoint& point) {
  ...
  stream << std::setprecision(std::numeric_limits<long double>::max_digits10) << '(' << point.x
         << ", " << point.y << ')';
  return stream.str();
}
...
inline double boundarySnapTolerance(double boundary) {
  ...
  const double upward_ulp =
    std::abs(std::nextafter(boundary, std::numeric_limits<double>::infinity()) - boundary);
  const double downward_ulp =
    std::abs(boundary - std::nextafter(boundary, -std::numeric_limits<double>::infinity()));
  return 2.0 * std::max(upward_ulp, downward_ulp);
}

inline long double canonicalizeGridBoundary(
  double world, double origin, double resolution, std::uint32_t extent, long double converted) {
  if (!std::isfinite(converted))
    return converted;

  const long double nearest = std::round(converted);
  if (!std::isfinite(nearest) || nearest < 0.0L || nearest > static_cast<long double>(extent)) {
    return converted;
  }

  const double reconstructed = origin + static_cast<double>(nearest) * resolution;
...
inline bool hasSeparatedWorldBoundarySnapWindows(double origin,
                                                 double resolution,
                                                 std::uint32_t extent) {
  ...
  for (std::uint64_t index = 1; index <= extent; ++index) {
    ...
    const long double separation =
      static_cast<long double>(current) - static_cast<long double>(previous);
    const long double combined_tolerance =
      static_cast<long double>(previous_tolerance) + static_cast<long double>(current_tolerance);
...
inline ValidationResult validateMap(const nav_msgs::msg::OccupancyGrid& map,
  ...
  const long double norm_squared = static_cast<long double>(orientation.x) * orientation.x +
  ...
  if (orientation.x != 0.0 || orientation.y != 0.0 || orientation.z != 0.0 ||
    ...
    return inconclusive("Collision oracle supports only an identity map rotation");
  }
  ...
  if (map.data.size() != expected_cells) {
    return inconclusive("OccupancyGrid data length does not match width * height");
  }
  ...
  for (std::size_t index = 0; index < map.data.size(); ++index) {
    ...
    if (value < -1 || value > 100) {
      ...
      return inconclusive("Unsupported occupancy value " + std::to_string(value) + " at cell (" +
  ...
  if (!hasSeparatedWorldBoundarySnapWindows(origin.position.x, resolution, map.info.width) ||
      !hasSeparatedWorldBoundarySnapWindows(origin.position.y, resolution, map.info.height)) {
    return inconclusive(
...
inline bool worldToGrid(const nav_msgs::msg::OccupancyGrid& map,
  ...
  const double resolution = static_cast<double>(map.info.resolution);
  const long double long_resolution = static_cast<long double>(resolution);
  long double x =
    (static_cast<long double>(world.x) - static_cast<long double>(map.info.origin.position.x)) /
    long_resolution;
  long double y =
    (static_cast<long double>(world.y) - static_cast<long double>(map.info.origin.position.y)) /
    long_resolution;
  x = canonicalizeGridBoundary(world.x, map.info.origin.position.x, resolution, map.info.width, x);
  y = canonicalizeGridBoundary(world.y, map.info.origin.position.y, resolution, map.info.height, y);
...
inline bool isFreeCell(const nav_msgs::msg::OccupancyGrid& map,
                       std::uint32_t x,
                       std::uint32_t y,
                       const OracleOptions& options) {
  // The planner deliberately forces the complete outer ring occupied,
  // independently of the input data values.
  if (x == 0 || y == 0 || x + 1 == map.info.width || y + 1 == map.info.height) {
    return false;
  }

  const std::size_t index = static_cast<std::size_t>(y) * map.info.width + x;
  const int value = static_cast<int>(map.data[index]);
  if (value == -1)
...
inline bool intersectClosedAxis(long double start,
...
inline bool segmentCellClosureInterval(const GridPoint& start,
                                       const GridPoint& end,
                                       std::uint32_t cell_x,
                                       std::uint32_t cell_y,
  ...
  if (!intersectClosedAxis(start.x,
                           dx,
                           static_cast<long double>(cell_x),
                           static_cast<long double>(cell_x) + 1.0L,
                           lower,
                           upper) ||
      !intersectClosedAxis(start.y,
                           dy,
                           static_cast<long double>(cell_y),
                           static_cast<long double>(cell_y) + 1.0L,
                           lower,
                           upper)) {
    return false;
  }
  ...
  if (lower > upper + tolerance)
    return false;
  if (lower > upper) {
    const long double contact = std::clamp((lower + upper) / 2.0L, 0.0L, 1.0L);
    lower = contact;
    upper = contact;
  }
  interval = ParameterInterval{std::clamp(lower, 0.0L, 1.0L), std::clamp(upper, 0.0L, 1.0L)};
  return true;
}

inline ValidationResult collisionResult(const GridPoint& start,
                                        const GridPoint& end,
                                        long double parameter,
                                        std::size_t segment_index,
                                        const std::string& reason) {
  const long double clamped = std::clamp(parameter, 0.0L, 1.0L);
  const GridPoint collision{start.x + clamped * (end.x - start.x),
  ...
  result.diagnostic = reason + " near grid coordinate " + formatGridPoint(collision);
  return result;
}

inline ValidationResult validateGridSegment(const nav_msgs::msg::OccupancyGrid& map,
  ...
  for (const ParameterInterval& interval : free_intervals) {
    if (!coverage_started) {
      ...
      if (interval.lower > tolerance) {
        return collisionResult(
          start, end, 0.0L, segment_index, "Segment starts outside the closure of every free cell");
      }
      coverage_started = true;
      covered_until = interval.upper;
    } else {
      const long double tolerance = parameterTolerance(covered_until, interval.lower);
      if (interval.lower > covered_until + tolerance) {
        return collisionResult(start,
                               end,
                               (covered_until + interval.lower) / 2.0L,
                               segment_index,
    ...
    if (covered_until >= 1.0L - parameterTolerance(covered_until, 1.0L)) {
      ValidationResult result;
      result.status = ValidationStatus::kCollisionFree;
      result.segment_index = segment_index;
      result.diagnostic = "Segment is completely covered by free-cell closures";
      return result;
    }
  }
  ...
  if (!coverage_started) {
    return collisionResult(
      start, end, 0.0L, segment_index, "Segment does not intersect the closure of any free cell");
  }
  return collisionResult(start,
                         end,
                         (covered_until + 1.0L) / 2.0L,
                         segment_index,
...
inline ValidationResult validateSegment(const nav_msgs::msg::OccupancyGrid& map,
  ...
  return detail::validateGridSegment(map, grid_start, grid_end, options, segment_index);
}

inline ValidationResult validatePath(const nav_msgs::msg::OccupancyGrid& map,
  ...
  if (path.header.frame_id != map.header.frame_id) {
    return detail::inconclusive("Path frame_id '" + path.header.frame_id +
  ...
  for (std::size_t index = 0; index < path.poses.size(); ++index) {
    if (path.poses[index].header.frame_id != map.header.frame_id) {
      return detail::inconclusive("Path pose " + std::to_string(index) + " frame_id '" +
  ...
  for (std::size_t index = 0; index + 1 < path.poses.size(); ++index) {
    ...
    std::string error;
    if (!detail::worldToGrid(map, path.poses[index].pose.position, start, error) ||
        !detail::worldToGrid(map, path.poses[index + 1].pose.position, end, error)) {
      ValidationResult result = detail::inconclusive(error);
      result.segment_index = index;
      return result;
    }
    ValidationResult result = detail::validateGridSegment(map, start, end, options, index);
...
inline SelfIntersectionResult validateNoSelfIntersection(const nav_msgs::msg::Path& path,
  ...
  for (std::size_t index = 0; index < path.poses.size(); ++index) {
    ...
    if (pose.header.frame_id != path.header.frame_id) {
      return detail::selfIntersectionInconclusive(
        "Path pose " + std::to_string(index) + " frame_id '" + pose.header.frame_id +
        "' does not match path frame_id '" + path.header.frame_id + "'");
    }
    const auto& position = pose.pose.position;
    if (!std::isfinite(position.x) || !std::isfinite(position.y) || !std::isfinite(position.z)) {
      return detail::selfIntersectionInconclusive("Path pose " + std::to_string(index) +
    ...
    points.emplace_back(detail::makeExactPlanarPoint(position.x, position.y));
  }
  ...
  for (std::size_t first = 0; first + 1 < points.size(); ++first) {
    ...
    for (std::size_t second = first + 2; second + 1 < points.size(); ++second) {
      const SelfIntersectionKind kind = detail::classifyClosedSegmentIntersection(
      ...
      result.diagnostic = std::string("Path has a ") + detail::selfIntersectionKindName(kind) +
                          " between segments " + std::to_string(first) + " and " +
  ...
  result.diagnostic = "No pair of non-consecutive path segments intersects";
  return result;
}
