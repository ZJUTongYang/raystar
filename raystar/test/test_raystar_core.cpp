...
static BoundaryEndpoint makeExactBoundaryEndpoint(
...
static VisibilityRegion makeIntegerBoundary(
  std::initializer_list<std::pair<int, int>> vertices)
{
  ...
  {
    boundary.emplace_back(makeExactBoundaryEndpoint(
...
{
  const VisibilityRegion rectangle = makeIntegerBoundary({
    {0, 0}, {4, 0}, {4, 4}, {0, 4}});
  const auto classify = [&](int x, int y) {
    return detail::classifyPointInVisibilityBoundary(
      rectangle, exact_geometry::Point(x, y));
  };
  ...
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    reversed, exact_geometry::Point(2, 2)), std::make_pair(true, false));
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    reversed, exact_geometry::Point(5, 2)), std::make_pair(false, false));
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    VisibilityRegion{rectangle[0], rectangle[1]}, exact_geometry::Point(2, 0)),
...
TEST(ExactPointLocation, CooperativeOverloadDistinguishesFailureAndStop)
{
  ...
  const VisibilityRegion invalid_boundary = makeIntegerBoundary({
    {0, 0}, {4, 0}});
  classification = {true, true};
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
      invalid_boundary, exact_geometry::Point(2, 1), classification,
      never_stop),
  ...
  for (int x = 0; x < 256; ++x)
  {
    long_boundary.emplace_back(makeExactBoundaryEndpoint(
      exact_geometry::Point(x, 0)));
  }

  size_t poll_count = 0;
  const StopToken stop_token([&poll_count]() {
    return ++poll_count == 8;
  });
  classification = {true, true};
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
      long_boundary, exact_geometry::Point(0, 1), classification,
      stop_token),
...
TEST(ExactPointLocation, ExplicitClosedModePreservesRectangleSemantics)
{
  ...
  const auto classify = [&](int x, int y) {
    ...
    const auto compatibility_result =
      detail::classifyPointInVisibilityBoundary(rectangle, query);
    const auto explicit_result = detail::classifyPointInVisibilityBoundary(
      rectangle, query, unused_source,
...
TEST(ExactPointLocation, OpenSectorClosesThroughSourceSpokes)
{
  ...
  const VisibilityRegion open_boundary = makeIntegerBoundary({
    {0, 4}, {4, 4}, {4, 0}});
  ...
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    open_boundary, near_source, source,
    detail::VisibilityBoundaryMode::open_sector),
    std::make_pair(true, false));
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    open_boundary, near_source), std::make_pair(false, false));

  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    open_boundary, source, source,
    detail::VisibilityBoundaryMode::open_sector),
    std::make_pair(false, true));
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    open_boundary, exact_geometry::Point(0, 2), source,
    detail::VisibilityBoundaryMode::open_sector),
    std::make_pair(false, true));
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    open_boundary, exact_geometry::Point(2, 0), source,
    detail::VisibilityBoundaryMode::open_sector),
    std::make_pair(false, true));
  EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
    open_boundary, exact_geometry::Point(5, 2), source,
    detail::VisibilityBoundaryMode::open_sector),
    std::make_pair(false, false));
}
...
TEST(ExactPointLocation, UsesExactEndpointsAtLargeCoordinates)
{
  ...
  const auto make_rectangle = [&](const FT& right_x) {
    return VisibilityRegion{
      makeExactBoundaryEndpoint(Point(coordinate - FT(2), FT(0))),
...
{
  VisibilityRegion boundary = makeIntegerBoundary({
    {0, 0}, {6, 0}, {6, 6}, {3, 6}, {3, 3}, {3, 6}, {0, 6}});
  const auto expect_classification = [&](const VisibilityRegion& candidate) {
    EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
      candidate, exact_geometry::Point(1, 3)), std::make_pair(true, false));
    EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
      candidate, exact_geometry::Point(4, 4)), std::make_pair(true, false));
    EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
      candidate, exact_geometry::Point(7, 4)), std::make_pair(false, false));
    EXPECT_EQ(detail::classifyPointInVisibilityBoundary(
      candidate, exact_geometry::Point(3, 4)), std::make_pair(false, true));
  };
...
TEST(ExactPointLocation, MatchesLegacyParityAwayFromBoundary)
{
  const std::vector<VisibilityRegion> boundaries = {
    ...
    makeIntegerBoundary({
      {0, 0}, {8, 0}, {4, 0}, {8, 4}, {8, 8}, {0, 8}}),
    makeIntegerBoundary({
      {0, 0}, {8, 0}, {8, 0}, {8, 8}, {4, 8}, {4, 4}, {4, 8}, {0, 8}})
  };

  const auto legacy_classify = [](const VisibilityRegion& boundary,
         ...
         i < boundary.size(); j = i++)
    {
      ...
      const bool crosses =
        (current.second > query_y) != (previous.second > query_y);
      if (!crosses)
        continue;
      const double intersection_x = current.first +
        (previous.first - current.first) *
        (query_y - current.second) / (previous.second - current.second);
    ...
    for (int x_quarters = -3; x_quarters <= 35; x_quarters += 2)
    {
      for (int y_quarters = -3; y_quarters <= 35; y_quarters += 2)
      {
        SCOPED_TRACE(testing::Message()
          ...
          << " query_quarters=(" << x_quarters << "," << y_quarters << ")");
        ...
        const auto exact_result = detail::classifyPointInVisibilityBoundary(
          boundaries[boundary_index], exact_query);
        if (exact_result.second)
          continue;
        EXPECT_EQ(exact_result, legacy_classify(
          boundaries[boundary_index], x_quarters / 4.0, y_quarters / 4.0));
      }
    }
  }
}
...
TEST(RaystarCore, ContinuousEndpointsRemainDistinctFromTheirCells)
{
  ...
  EXPECT_NEAR(first.path_solutions.front().path_cost_,
    std::hypot(goal.first - 2.25, goal.second - 3.25), 1e-9);
  ...
  EXPECT_NE(second.path_solutions.front().path_cost_,
    first.path_solutions.front().path_cost_);
  for (size_t index = 1; index + 1 < second_points.size(); ++index)
  {
    EXPECT_DOUBLE_EQ(second_points[index].first,
      std::round(second_points[index].first));
    EXPECT_DOUBLE_EQ(second_points[index].second,
      std::round(second_points[index].second));
  }
}
...
TEST(RaystarCore, ContinuousEndpointValidationReportsNonFiniteCoordinates)
{
  ...
  const auto result = core.plan(
    makeOpenMap(), Point2d{std::numeric_limits<double>::quiet_NaN(), 2.5},
    Point2d{17.5, 16.5}, 1, false);
...
TEST(RaystarCore, StartEqualsGoalReturnsZeroLengthContinuousPath)
{
  ...
  const auto result = core.plan(
    makeOpenMap(), endpoint, endpoint, 1, false);
...
TEST(RaystarCore, RejectsContinuousEndpointOnFinalBoundary)
{
  RaystarCore core;
  const auto result = core.plan(
    makeOpenMap(), Point2d{2.5, 1.0}, Point2d{17.5, 10.5}, 1, false);
  EXPECT_FALSE(result.success);
  EXPECT_NE(result.message.find("start"), std::string::npos)
    << result.message;
...
TEST(RaystarCore, RejectsOccupiedAndForcedBorderEndpointsSymmetrically)
{
  const auto map_with_occupied_cell = [](int x, int y) {
    auto map = makeOpenMap();
    map.data[static_cast<size_t>(y) * map.width +
  ...
  const std::vector<InvalidEndpointCase> invalid_cases = {
    {"occupied start", map_with_occupied_cell(5, 5),
      Point2d{5.5, 5.5}, Point2d{17.5, 17.5},
      "Invalid start: corresponding occupancy cell is occupied"},
    {"occupied goal", map_with_occupied_cell(15, 15),
      Point2d{2.5, 2.5}, Point2d{15.5, 15.5},
      "Invalid goal: corresponding occupancy cell is occupied"},
    {"forced-border start", open_map,
      Point2d{0.5, 10.5}, Point2d{17.5, 10.5},
      "Invalid start: corresponding cell is occupied or on the map boundary"},
    {"forced-border goal", open_map,
      Point2d{2.5, 10.5}, Point2d{19.5, 10.5},
  ...
  for (const auto& invalid_case : invalid_cases)
  {
    ...
    const auto result = core.plan(
      invalid_case.map, invalid_case.start, invalid_case.goal, 1, false);
    EXPECT_FALSE(result.success);
    ...
    const auto recovered = core.plan(
      makeOpenMap(), Point2d{2.5, 2.5}, Point2d{17.5, 17.5}, 1, false);
...
TEST(RaystarCore, ContinuousGoalAffectsObstacleRouteAndCost)
{
  ...
  const auto first = core.plan(
    map, start, Point2d{25.25, 15.25}, 1, false);
  ASSERT_TRUE(first.success) << first.message;
  const auto second = core.plan(
    map, start, Point2d{25.75, 15.75}, 1, false);
  ASSERT_TRUE(second.success) << second.message;
  ASSERT_EQ(first.path_solutions.size(), 1u);
  ASSERT_EQ(second.path_solutions.size(), 1u);
  ...
  EXPECT_NE(first.path_solutions.front().path_cost_,
    second.path_solutions.front().path_cost_);
  ...
  EXPECT_NEAR(first.path_solutions.front().path_cost_,
    path_length(first.path_solutions.front()), 1e-9);
  EXPECT_NEAR(second.path_solutions.front().path_cost_,
    path_length(second.path_solutions.front()), 1e-9);
...
TEST(RaystarCore, PathSolutionsRetainCompleteDeepAncestorChains)
{
       ...
       solution_index < result.path_solutions.size(); ++solution_index)
  {
    ...
    for (size_t chain_position = 0;
         chain_position < chain.size(); ++chain_position)
    {
    ...
    std::reverse(
      reconstructed_reverse.begin(), reconstructed_reverse.end());
...
TEST(RaystarCore, EveryExpandedNodeRetainsItsCompleteAncestorPrefix)
{
  RaystarCore core;
  const auto result = core.plan(
    makeSimpleMap(), Point2d{5.25, 15.25}, Point2d{25.75, 15.75},
    2, false);
...
TEST(RaystarCore, ReportsUnsupportedSharedVertexTopologyBeforePlanning)
{
  ...
  EXPECT_EQ(result.polymap, nullptr);
  EXPECT_NE(result.message.find("Map geometry construction failed"),
    std::string::npos) << result.message;
  EXPECT_NE(result.message.find("Unsupported obstacle topology"),
    std::string::npos) << result.message;
  EXPECT_NE(result.message.find("(3, 3)"), std::string::npos)
    << result.message;
  EXPECT_EQ(result.limit_reached, PlanningLimitReached::none);
  EXPECT_TRUE(result.path_solutions.empty());
  EXPECT_TRUE(core.getNodes().empty());
...
TEST(RaystarCore, RejectsMalformedGridMap)
{
  ...
  EXPECT_EQ(result.message,
    "Invalid map: width and height must fit signed int indexing");
  EXPECT_TRUE(core.getNodes().empty());
...
TEST(RaystarCore, EnforcesMapCellAdmissionBeforePlanningStateIsAllocated)
{
  ...
  const auto rejected = core.plan(map,
    PlanEndpoint(2, 2, 2.25, 2.25),
    PlanEndpoint(17, 17, 17.75, 17.75),
    1, false, limits);

  EXPECT_FALSE(rejected.success);
  EXPECT_EQ(rejected.message,
    "Invalid map: cell count 400 exceeds max_map_cells=399");
...
TEST(RaystarCore, MapByteAdmissionUsesCheckedWorkingSetEstimate)
{
  ...
  const size_t estimated_bytes =
    map.data.size() * kEstimatedPlannerMapBytesPerCell;
  RaystarCore core;
  ...
  EXPECT_FALSE(rejected.success);
  EXPECT_NE(rejected.message.find("max_map_bytes"), std::string::npos)
    << rejected.message;
  EXPECT_EQ(rejected.polymap, nullptr);
  EXPECT_TRUE(core.getNodes().empty());
...
TEST(RaystarCore, RejectsOutOfBoundsCoordinates)
{
  ...
  EXPECT_FALSE(result.success);
  EXPECT_EQ(result.message,
    "Invalid start: coordinates (-1, 2) are outside map bounds");

  result = core.plan(map, 20, 2, 17, 17, 1, false);
  EXPECT_FALSE(result.success);
  EXPECT_EQ(result.message,
    "Invalid start: coordinates (20, 2) are outside map bounds");

  result = core.plan(map, 2, 2, 17, -1, 1, false);
  EXPECT_FALSE(result.success);
  EXPECT_EQ(result.message,
    "Invalid goal: coordinates (17, -1) are outside map bounds");

  result = core.plan(map, 2, 2, 17, 20, 1, false);
  EXPECT_FALSE(result.success);
  EXPECT_EQ(result.message,
    "Invalid goal: coordinates (17, 20) are outside map bounds");
  EXPECT_TRUE(core.getNodes().empty());
}
...
TEST(RaystarCore, DeepCooperativeCancellationDoesNotCommitScopedNodeAndRecovers)
{
  ...
  const auto baseline = calibration_core.plan(
    map, 5, 15, 25, 15, 1, false, calibration_limits);
  ASSERT_TRUE(baseline.success) << baseline.message;
  ...
  const auto cancelled = core.plan(
    map, 5, 15, 25, 15, 1, false, cancellation_limits);
...
TEST(RaystarCore, ExternalAtomicCancellationFromAnotherThreadStopsAndRecovers)
{
  ...
  {
    std::unique_lock<std::mutex> lock(poll_mutex);
    observed_poll = poll_condition.wait_for(
      lock, std::chrono::seconds(5), [&]() { return planner_polled; });
  }
...
TEST(RaystarCore, NodeLimitReturnsFoundPathsAndMarksSearchIncomplete)
{
  ...
  PlanningLimits limits;
  limits.max_nodes = static_cast<size_t>(
    baseline.path_solutions.front().path_node_index_.back() + 1);
...
TEST(Node, GenerateChildRejectsDegenerateVisibility)
{
  ...
  Node empty_node(&poly, 0, 5, 5, 0.0, 0.0,
    empty_visibility, empty_topology);
  ...
  const std::vector<std::pair<double, double>> visibility = {
    {6.0, 5.0}, {7.0, 5.0}
  };
  const std::vector<std::pair<int, int>> mismatched_topology = {{-1, -1}};
  Node mismatched_node(&poly, 1, 5, 5, 0.0, 0.0,
    visibility, mismatched_topology);
...
TEST(Node, CooperativeStopDoesNotCommitFullVisibilityOrChildren)
{
  ...
  for (size_t index = 0; index < original_full_visibility.size(); ++index)
  {
    ...
    EXPECT_EQ(node.full_visibility_region_[index].support,
      original_full_visibility[index].support);
  }
  ...
  ASSERT_EQ(node.generateChild(&poly, child_count_only, &error),
  ...
  const StopToken child_stop_at_last(
    [&child_stopped_poll_count, child_full_poll_count]() {
      return ++child_stopped_poll_count >= child_full_poll_count;
    });
  EXPECT_EQ(node.generateChild(&poly, child_stop_at_last, &error),
    OperationStatus::stopped);
...
TEST(Node, GenerateChildRejectsCoordinatesOutsideIntRange)
{
  ...
  const std::vector<std::vector<std::pair<double, double>>> invalid_regions = {
    {{above_int, 5.0}, {above_int + 1024.0, 5.0}},
    {{below_int, 5.0}, {below_int - 1024.0, 5.0}}
  };
  const std::vector<std::pair<int, int>> gap_topology = {
    {0, 0}, {-1, -1}
  };
  ...
  {
    SCOPED_TRACE(i);
    Node node(&poly, static_cast<int>(i), 5, 5, 0.0, 0.0,
      invalid_regions[i], gap_topology);
...
TEST(Node, LegacyAdapterRejectsNonFiniteCoordinatesBeforeExactConstruction)
{
  ...
  const std::vector<double> invalid_coordinates = {
  ...
  for (size_t i = 0; i < invalid_coordinates.size(); ++i)
  {
    SCOPED_TRACE(i);
    const std::vector<std::pair<double, double>> visibility = {
      {invalid_coordinates[i], 5.0}, {7.0, 5.0}
    };
    Node node(&poly, static_cast<int>(i), 5, 5, 0.0, 0.0,
      visibility, topology);
...
TEST(Node, ConstructorsSafelyRejectInvalidSeeds)
{
  ...
  Node root_node(&poly, 0, std::numeric_limits<double>::infinity(), 5.0,
    0.0, 0.0, visibility, topology);
  std::string error;
  ...
  const double below_int = static_cast<double>(std::numeric_limits<int>::lowest()) - 1024.0;
  Node scoped_node(&poly, 1, below_int, 5.0, 0.0, 0.0, 0,
    visibility, topology);
  error.clear();
...
TEST(Child, ExactEndpointProjectionRoundsCanonicalGridCoordinate)
{
  ...
  EXPECT_EQ(node.visibility_region_[0].position,
    (Point2d{static_cast<double>(coordinate.first),
...
TEST(Node, GenerateChildHandlesUnknownAndRejectsInvalidTopology)
{
  ...
  const std::vector<std::pair<int, int>> unknown_topology = {
    {-1, -1}, {-1, -1}
  };
  Node unknown_node(&poly, 0, 5, 5, 0.0, 0.0,
    visibility, unknown_topology);
  ...
  const std::vector<std::pair<int, int>> half_invalid_topology = {
    {-1, 0}, {-1, -1}
  };
  Node invalid_node(&poly, 1, 5, 5, 0.0, 0.0,
    visibility, half_invalid_topology);
  ...
  Node out_of_range_node(&poly, 2, 5, 5, 0.0, 0.0,
    visibility, out_of_range_topology);
