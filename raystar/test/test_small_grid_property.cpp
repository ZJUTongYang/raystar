...
SmallGridCase makeUniformCase(
  std::uint32_t width, std::uint32_t height, GridCell start,
  GridCell goal, bool allow_unknown, std::int8_t value = 0)
{
  ...
  property_case.map.data.assign(
    static_cast<std::size_t>(width) * height, value);
...
std::size_t flatIndex(const SmallGridCase & property_case, GridCell cell)
{
  return static_cast<std::size_t>(cell.y) *
         property_case.map.info.width + cell.x;
}

bool generatedCasesEqual(
  const SmallGridCase & lhs, const SmallGridCase & rhs)
{
  return lhs.case_index == rhs.case_index &&
         lhs.seed == rhs.seed && lhs.map == rhs.map &&
         lhs.start == rhs.start && lhs.goal == rhs.goal &&
...
TEST(SmallGridProperty, FixedSeedCorpusSatisfiesOraclePreconditions)
{
  ...
  for (std::size_t case_index = 0; case_index < kCorpusSize; ++case_index)
  {
    ...
    EXPECT_FALSE(isForcedOuterRing(
    ...
    EXPECT_EQ(
      fourNeighborReachable(property_case),
      property_case.expected_reachable);

    const std::size_t combination =
      (property_case.allow_unknown ? 2U : 0U) +
      (property_case.expected_reachable ? 1U : 0U);
...
{
  const SmallGridCase property_case = makeUniformCase(
    5, 5, GridCell{1, 1}, GridCell{3, 3}, false);
  ...
  for (std::uint32_t y = 0; y < property_case.map.info.height; ++y)
  {
    for (std::uint32_t x = 0; x < property_case.map.info.width; ++x)
    {
      if (!isForcedOuterRing(property_case.map, x, y))
        continue;
      EXPECT_EQ(
        property_case.map.data[
          flatIndex(property_case, GridCell{x, y})],
        0);
      EXPECT_TRUE(isEffectivelyBlocked(
        property_case.map, x, y, property_case.allow_unknown));
    }
  }
...
{
  SmallGridCase property_case = makeUniformCase(
    7, 5, GridCell{1, 2}, GridCell{5, 2}, false);
  ...
  EXPECT_TRUE(isEffectivelyBlocked(
    property_case.map, 3, 2, false, kOccupiedThreshold));
  EXPECT_FALSE(isEffectivelyBlocked(
    property_case.map, 3, 1, false, kOccupiedThreshold));
...
{
  SmallGridCase property_case = makeUniformCase(
    7, 7, GridCell{1, 1}, GridCell{5, 5}, false);
