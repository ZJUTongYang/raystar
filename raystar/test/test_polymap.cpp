...
public:
  ...
  static bool reconstructWithValidator(
    Polymap& polymap, Validator validator)
  {
    return polymap.constructCGALRelated(
      validator, polymap.construction_error_);
  }
  ...
  static bool refreshObstacles(
    Polymap& polymap, int start_x, int start_y, int goal_x, int goal_y)
  {
  ...
  {
    return Polymap(
      map, start_x, start_y, goal_x, goal_y, StopToken{});
  }
  ...
  {
    return {
      polymap.vertices_location_x_flat_.size(),
      polymap.vertices_location_y_flat_.size()};
  }
  ...
  static bool reverseCachedVisibility(
    Polymap& polymap, int source_x, int source_y)
  {
    ...
    if (cached == polymap.visibility_storage_.end())
      return false;
    std::reverse(
      cached->second.vertices.begin(), cached->second.vertices.end());
    return true;
  }
  ...
  static bool validateObstacleTopology(
    const Polymap& polymap, std::string& error)
  {
    return polymap.validateObstacleTopology(error, true);
  }

  static bool simplificationChordIsTopologicallySafe(
    const Polymap& polymap, size_t obstacle_index, size_t current_index)
  {
    return polymap.simplificationChordIsTopologicallySafe(
      obstacle_index, current_index);
  }
};
...
Polymap makeReadyPolymap(Args&&... args)
{
  ...
  if (!result)
  {
    throw std::runtime_error(
      result.error.empty() ? "Polymap construction did not produce a value" :
      result.error);
  }
...
bool throwFromCDTValidator(const PolymapTestPeer::CDT&)
{
  throw CGAL::Assertion_exception(
    "CGAL", "injected validator assertion", __FILE__, __LINE__,
...
GridBoundaryEdge makeGridBoundaryEdge(
  const GridVertex& first, const GridVertex& second)
{
  if (first <= second)
    return {first, second};
  return {second, first};
  ...
  for (const auto& obstacle : obstacles)
  {
    ...
    for (size_t index = 0; index < vertices.size(); ++index)
    {
      ...
      result.all_segments_are_unit_length =
        result.all_segments_are_unit_length &&
        std::abs(from.first - to.first) +
        std::abs(from.second - to.second) == 1;
...
std::set<GridBoundaryEdge> collectExpectedReachableBoundaryEdges(
  const GridMap& map, int start_x, int start_y)
{
  ...
  while (!pending.empty())
  {
    ...
    if (visited[static_cast<size_t>(index)] != 0 ||
        map.data[static_cast<size_t>(index)] != 0)
    {
      continue;
    }
    visited[static_cast<size_t>(index)] = 1;

    const auto visit_or_record = [&](int neighbor_x, int neighbor_y,
        const GridVertex& edge_from, const GridVertex& edge_to) {
...
TEST(VisibilityPredicates, ExactCaseClassifierMatchesLegacyAwayFromBoundaries)
{
  ...
  const auto legacy_classify = [](const auto& lower, const auto& upper,
      const auto& candidate) {
    const double lower_angle = std::atan2(lower.second, lower.first);
    const double upper_angle = lower_angle + normalize_angle_positive(
      std::atan2(upper.second, upper.first) - lower_angle);
    const double theta = lower_angle + normalize_angle(
      std::atan2(candidate.second, candidate.first) - lower_angle);
...
TEST(VisibilityPredicates, PolarComparatorsPreserveWrapAndSameRayOrder)
{
  ...
  std::stable_sort(standard.begin(), standard.end(), [&](const Point& first,
      const Point& second) {
  ...
  std::stable_sort(near_then_far.begin(), near_then_far.end(), [&](const Point& first,
      const Point& second) {
    return exact_geometry::compareRaysLikeAtan2(source, first, second) == CGAL::SMALLER;
  });
  EXPECT_EQ(near_then_far, (std::vector<Point>{near, far}));
  std::vector<Point> far_then_near = {far, near};
  std::stable_sort(far_then_near.begin(), far_then_near.end(), [&](const Point& first,
      const Point& second) {
    return exact_geometry::compareRaysLikeAtan2(source, first, second) == CGAL::SMALLER;
  });
  ...
  std::vector<Point> relative = {
    Point(1, 0), Point(0, -1), Point(-1, 0), reference};
  std::stable_sort(relative.begin(), relative.end(), [&](const Point& first,
      const Point& second) {
    return exact_geometry::compareRaysCounterClockwiseFrom(
      source, reference, first, second) == CGAL::SMALLER;
  });
  EXPECT_EQ(relative,
    (std::vector<Point>{reference, Point(-1, 0), Point(0, -1), Point(1, 0)}));
  ...
  EXPECT_EQ(exact_geometry::compareRaysCounterClockwiseFrom(
    source, reference, same_reference_ray_near, same_reference_ray_far), CGAL::EQUAL);
  EXPECT_EQ(exact_geometry::compareRaysCounterClockwiseFrom(
    source, reference, same_reference_ray_far, same_reference_ray_near), CGAL::EQUAL);
}
...
TEST(VisibilityPredicates, SegmentRayIntersectionUsesExactParameterDomains)
{
  ...
  const auto large_intersection = exact_geometry::intersectSegmentWithRay(
    Point(1000000000, -1000000000), Point(1000000000, 1000000000),
    Point(0, 0), Point(3, 1));
  ...
  EXPECT_FALSE(exact_geometry::intersectSegmentWithRay(
    Point(-2, -1), Point(-2, 1), Point(0, 0), Point(1, 0)).has_value())
    << "A line hit behind the ray source is not a visibility intersection";
  EXPECT_FALSE(exact_geometry::intersectSegmentWithRay(
    Point(2, 1), Point(2, 2), Point(0, 0), Point(1, 0)).has_value())
    << "The infinite blocker line must not extend beyond its segment";
  EXPECT_FALSE(exact_geometry::intersectSegmentWithRay(
    Point(0, 1), Point(3, 1), Point(0, 0), Point(1, 0)).has_value());
  EXPECT_FALSE(exact_geometry::intersectSegmentWithRay(
    Point(2, 0), Point(3, 0), Point(0, 0), Point(1, 0)).has_value())
    << "A collinear overlap has no unique intersection endpoint";
  EXPECT_FALSE(exact_geometry::intersectSegmentWithRay(
    Point(2, 0), Point(2, 0), Point(0, 0), Point(1, 0)).has_value());
  EXPECT_FALSE(exact_geometry::intersectSegmentWithRay(
    Point(2, -1), Point(2, 1), Point(0, 0), Point(0, 0)).has_value());
}
...
TEST(VisibilityPredicates, ClosedCounterClockwiseSweepSupportsWideWrap)
{
  ...
  EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, end, start));
  EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, end, Point(-1, 0)));
  EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, end, Point(0, -1)));
  EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, end, Point(1, 0)));
  EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, end, end));
  EXPECT_FALSE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, end, Point(0, 1)));

  EXPECT_TRUE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, Point(-2, 2), Point(-3, 3)));
  EXPECT_FALSE(exact_geometry::isClosedCounterClockwiseSweepMember(
    source, start, Point(-2, 2), Point(1, 0)));
}
...
TEST(VisibilityPredicates, CollinearMiddleIsRemovableButSpikeIsNot)
{
  ...
  EXPECT_FALSE(exact_geometry::isRemovableCollinearMiddle(
    previous, middle, Point(2000000000, 1999999999)));
}
...
TEST(Polymap, RejectsMalformedGridBeforeCopyingOrSizingRegistries)
{
  ...
  EXPECT_EQ(PolymapTestPeer::registrySizes(integer_endpoints),
    (std::pair<size_t, size_t>{0, 0}));

  Polymap continuous_endpoints(
    malformed, 2, 2, 17, 17,
    Point2d{2.25, 2.25}, Point2d{17.75, 17.75});
...
TEST(Polymap, RejectsUnrepresentableCellCountBeforeRegistryResize)
{
  ...
  EXPECT_EQ(PolymapTestPeer::registrySizes(polymap),
    (std::pair<size_t, size_t>{0, 0}));
}
...
TEST(Polymap, CooperativeConstructionStopClearsAllDerivedState)
{
  ...
  const StopToken stop_at_last_poll(
    [&stopped_poll_count, full_poll_count]() {
      return ++stopped_poll_count >= full_poll_count;
    });
...
TEST(Polymap, OddWidthBoundaryEdgesWithEqualLegacySumsRemainDistinct)
{
  ...
  EXPECT_EQ(extracted.edges.count(
    makeGridBoundaryEdge({1, 1}, {1, 2})), 1u);
  EXPECT_EQ(extracted.edges.count(
    makeGridBoundaryEdge({3, 1}, {4, 1})), 1u);
}
...
TEST(Polymap, SharedObstacleVertexIsRejectedBeforeRegistryAndCDT)
{
  ...
  EXPECT_FALSE(polymap.isCDTReady());
  EXPECT_TRUE(polymap.getCDTEdges().empty());
  EXPECT_NE(PolymapTestPeer::constructionError(polymap).find(
...
TEST(Polymap, RawBoundaryEndpointIsRejectedBeforeTopologySimplification)
{
  ...
  Polymap poly(
    map, 3, 2, 5, 5, Point2d{3.0, 2.5}, Point2d{5.5, 5.5});
  EXPECT_TRUE(poly.hasSolution());
  EXPECT_FALSE(poly.isCDTReady());
  EXPECT_NE(PolymapTestPeer::constructionError(poly).find("Invalid start position"),
    std::string::npos) << PolymapTestPeer::constructionError(poly);
  EXPECT_NE(PolymapTestPeer::constructionError(poly).find("boundary"),
    std::string::npos) << PolymapTestPeer::constructionError(poly);
}
...
TEST(Polymap, TopologyValidatorReportsSharedObstacleIdsAndCoordinate)
{
  ...
  polymap.obstacles() = {
    Obs{{{0, 0}, {0, 10}, {10, 10}, {10, 0}}},
    Obs{{{2, 2}, {4, 2}, {4, 4}, {2, 4}}},
    Obs{{{4, 4}, {6, 4}, {6, 6}, {4, 6}}}};

  std::string error;
  EXPECT_FALSE(PolymapTestPeer::validateObstacleTopology(polymap, error));
  EXPECT_NE(error.find("obstacles 1 and 2 share vertex"), std::string::npos)
    << error;
...
TEST(Polymap, SimplificationChordRejectsNewIntersection)
{
  ...
  polymap.obstacles() = {
    Obs{{{2, 2}, {4, 4}, {6, 2}, {6, 6}, {2, 6}}}};
  EXPECT_TRUE(PolymapTestPeer::simplificationChordIsTopologicallySafe(
    polymap, 0, 1));

  polymap.obstacles().push_back(
    Obs{{{4, 1}, {5, 3}, {3, 3}}});
  EXPECT_FALSE(PolymapTestPeer::simplificationChordIsTopologicallySafe(
    polymap, 0, 1));
}
...
TEST(Polymap, CdtValidatorFalseProducesAtomicConstructionFailure)
{
  ...
  ASSERT_TRUE(polymap.getVisibilityRegion(
    5, 5, initial_visibility, &error)) << error;
  ASSERT_GT(PolymapTestPeer::visibilityCacheSize(polymap), 0u);

  EXPECT_FALSE(PolymapTestPeer::reconstructWithValidator(
    polymap, &rejectCDT));
  ...
  VisibilityRegion visibility = {
    BoundaryEndpoint{{-1.0, -1.0}, std::monostate{}}};
  error = "stale error";
  ...
  visibility = {
    BoundaryEndpoint{{-2.0, -2.0}, std::monostate{}}};
...
TEST(Polymap, CdtValidatorCGALFailureProducesAtomicConstructionFailure)
{
  ...
  EXPECT_NO_THROW(reconstruction_succeeded =
    PolymapTestPeer::reconstructWithValidator(
      polymap, &throwFromCDTValidator));

  EXPECT_FALSE(reconstruction_succeeded);
  EXPECT_TRUE(polymap.hasSolution());
  ASSERT_FALSE(PolymapTestPeer::constructionError(polymap).empty());
  EXPECT_NE(
    PolymapTestPeer::constructionError(polymap).find("injected validator assertion"),
    std::string::npos);
  expectClearedCDTState(polymap);
...
TEST(Polymap, VisibilityRegionComputedAndCachedAtomically)
{
  ...
  VisibilityRegion rich_visibility = {
    BoundaryEndpoint{{-100.0, -100.0}, std::monostate{}}};
  ...
  ASSERT_TRUE(poly.getVisibilityRegion(
    5, 5, legacy_visibility, legacy_topology, &error)) << error;
  ...
  for (size_t i = 0; i < rich_visibility.size(); ++i)
  {
    ...
    if (const auto* vertex =
        std::get_if<ObstacleVertexId>(&rich_visibility[i].support))
    {
      EXPECT_EQ(legacy_topology[i],
        std::make_pair(vertex->obstacle, vertex->vertex));
    }
    else
    {
...
TEST(Polymap, ContinuousRootVisibilityIsSourceSpecificAndUncached)
{
  ...
  std::string error;
  ASSERT_TRUE(poly.getRootVisibilityRegion(
    Point2d{5.25, 5.25}, first, &error)) << error;
  ASSERT_TRUE(poly.getRootVisibilityRegion(
    Point2d{5.75, 5.75}, second, &error)) << error;
  ...
  if (!differs)
  {
    for (size_t i = 0; i < first.size(); ++i)
    {
      if (first[i].position != second[i].position ||
          !(first[i].support == second[i].support))
      {
        differs = true;
        break;
      }
...
TEST(Polymap, CooperativeVisibilityStopDoesNotCommitOutputOrCache)
{
  ...
  ASSERT_EQ(poly.getVisibilityRegion(
      5, 5, baseline, count_only, &error),
  ...
  EXPECT_EQ(poly.getVisibilityRegion(
      5, 5, visibility, stop_at_last_poll, &error),
...
TEST(Polymap, StoppedCachedVisibilityValidationKeepsValidCache)
{
  ...
  ASSERT_EQ(poly.getVisibilityRegion(
      5, 5, cached_baseline, count_only, &error),
    OperationStatus::success) << error;
  ASSERT_GT(full_poll_count, 0u);

  VisibilityRegion stopped_output = {
    BoundaryEndpoint{{-100.0, -100.0}, std::monostate{}}};
  error = "stale error";
  size_t stopped_poll_count = 0;
  const StopToken stop_at_last_poll(
  ...
  EXPECT_EQ(poly.getVisibilityRegion(
      5, 5, stopped_output, stop_at_last_poll, &error),
    OperationStatus::stopped);
  EXPECT_EQ(stopped_poll_count, full_poll_count);
...
TEST(Polymap, NonFiniteRichEndpointIsRejectedWithoutExactConstruction)
{
  ...
  const BoundaryEndpoint endpoint{
    {std::numeric_limits<double>::quiet_NaN(), 5.0}, std::monostate{}};
...
TEST(Polymap, RichBoundarySupportIsDirectionalAndCannotBeDropped)
{
  ...
  for (size_t obstacle_index = 0;
       obstacle_index < poly.obstacles().size() && !found_long_edge;
       ++obstacle_index)
  {
  ...
  const auto interpolate = [&](double parameter) {
    return Point2d{
      from.first + parameter * (to.first - from.first),
  ...
  const BoundaryEndpoint exact_to{
    {static_cast<double>(to.first), static_cast<double>(to.second)}, edge.to};
  ...
  EXPECT_FALSE(poly.boundarySupportsConsecutive(
    BoundaryEndpoint{{5.5, 5.5}, std::monostate{}}, exact_from));
}
