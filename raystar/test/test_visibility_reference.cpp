...
Polymap makeReadyPolymap(const GridMap& map, int start_x, int start_y, int goal_x, int goal_y) {
...
enum class DomainLocation { blocked, free, boundary };
...
int angularHalf(const Direction& direction) {
  return direction.y > FT(0) || (direction.y == FT(0) && direction.x >= FT(0)) ? 0 : 1;
}
...
std::optional<RaySegmentHit> intersectRaySegment(const Point& source,
                                                 const Direction& direction,
                                                 const Segment& segment,
                                                 bool& collinear) {
  collinear = false;
  const Direction edge{segment.to.x() - segment.from.x(), segment.to.y() - segment.from.y()};
  if (isZero(edge))
    return std::nullopt;

  const Direction offset{segment.from.x() - source.x(), segment.from.y() - source.y()};
  const FT determinant = cross(direction, edge);
  if (determinant == FT(0)) {
    if (cross(offset, direction) == FT(0)) {
      const FT squared_length = dot(direction, direction);
      const Direction to_offset{segment.to.x() - source.x(), segment.to.y() - source.y()};
  ...
  if (ray_parameter < FT(0) || segment_parameter < FT(0) || segment_parameter > FT(1)) {
    return std::nullopt;
  }
  return RaySegmentHit{ray_parameter, segment_parameter};
}

RayCastResult castRay(const Point& source,
                      const Direction& direction,
                      const std::vector<Segment>& segments) {
  RayCastResult result;
  for (const auto& segment : segments) {
    bool collinear = false;
    const auto hit = intersectRaySegment(source, direction, segment, collinear);
    result.collinear = result.collinear || collinear;
    if (!hit || hit->ray_parameter <= FT(0))
      continue;
    if (hit->segment_parameter == FT(0) || hit->segment_parameter == FT(1)) {
      result.endpoint_hit = true;
    }
    result.positive_parameters.emplace_back(hit->ray_parameter);
  }

  std::sort(result.positive_parameters.begin(), result.positive_parameters.end());
  result.positive_parameters.erase(
    std::unique(result.positive_parameters.begin(), result.positive_parameters.end()),
...
std::vector<Segment> obstacleSegments(const Polymap& polymap) {
  ...
  for (const auto& obstacle : polymap.obstacles()) {
    ...
    for (size_t index = 0; index < vertices.size(); ++index) {
      ...
      segments.push_back(Segment{Point(from.first, from.second), Point(to.first, to.second)});
    }
  }
  return segments;
}

std::vector<Segment> visibilityBoundarySegments(const std::vector<Segment>& obstacle_segments,
                                                const Point& source,
  ...
  const bool open_sector = std::any_of(
    obstacle_segments.begin(), obstacle_segments.end(), [&source](const Segment& segment) {
  ...
  for (size_t index = 1; index < visibility_region.size(); ++index) {
    segments.push_back(
      Segment{exactPoint(visibility_region[index - 1]), exactPoint(visibility_region[index])});
  }
  ...
  if (open_sector) {
    ...
    segments.push_back(
      Segment{exactPoint(visibility_region.back()), exactPoint(visibility_region.front())});
  }
  return segments;
}

DomainLocation locateInFreeDomain(const Point& query,
                                  const std::vector<Segment>& obstacle_segments) {
  bool inside = false;
  for (const auto& segment : obstacle_segments) {
    const auto exact_segment = exact_geometry::Kernel::Segment_2(segment.from, segment.to);
    ...
    const FT intersection_x = segment.from.x() + (segment.to.x() - segment.from.x()) *
...
std::vector<Direction> eventDirections(const Point& source,
                                       const std::vector<Segment>& obstacle_segments,
                                       const VisibilityRegion& visibility_region) {
  std::vector<Direction> directions = {
    {FT(1), FT(0)}, {FT(0), FT(1)}, {FT(-1), FT(0)}, {FT(0), FT(-1)}};
  for (const auto& segment : obstacle_segments) {
  ...
  for (const auto& direction : directions) {
    if (unique_directions.empty() || !sameRay(unique_directions.back(), direction)) {
...
std::vector<Direction> sectorProbeDirections(const std::vector<Direction>& events) {
  std::vector<Direction> probes;
  probes.reserve(events.size() * 2);
  for (size_t index = 0; index < events.size(); ++index) {
    const Direction first = l1Normalized(events[index]);
    const Direction second = l1Normalized(events[(index + 1) % events.size()]);
    probes.push_back(Direction{FT(2) * first.x + second.x, FT(2) * first.y + second.y});
    probes.push_back(Direction{first.x + FT(2) * second.x, first.y + FT(2) * second.y});
  }
  return probes;
}

void expectVisibilityMatchesIndependentRayOracle(Polymap& polymap, int source_x, int source_y) {
  VisibilityRegion visibility_region;
  std::string error;
  ASSERT_TRUE(polymap.getVisibilityRegion(source_x, source_y, visibility_region, &error)) << error;
  ...
  const auto boundary_segments =
    visibilityBoundarySegments(obstacle_segments, source, visibility_region);
  const auto events = eventDirections(source, obstacle_segments, visibility_region);
  ...
  for (const auto& direction : probes) {
    SCOPED_TRACE(testing::Message() << "source=(" << source_x << "," << source_y << ")"
                                    << " direction=(" << CGAL::to_double(direction.x) << ","
                                    << CGAL::to_double(direction.y) << ")");

    const auto obstacle_cast = castRay(source, direction, obstacle_segments);
    ASSERT_FALSE(obstacle_cast.collinear) << "Probe generation must avoid obstacle-edge event rays";
    ASSERT_FALSE(obstacle_cast.endpoint_hit) << "Probe generation must avoid obstacle vertices";
    ...
    const Point midpoint(source.x() + direction.x * reference_extent / FT(2),
                         source.y() + direction.y * reference_extent / FT(2));
    const DomainLocation midpoint_location = locateInFreeDomain(midpoint, obstacle_segments);
    ASSERT_NE(midpoint_location, DomainLocation::boundary);
    const bool reference_visible = midpoint_location == DomainLocation::free;

    const auto boundary_cast = castRay(source, direction, boundary_segments);
    ASSERT_FALSE(boundary_cast.collinear)
      << "Probe generation must avoid visibility-boundary event rays";
    ASSERT_FALSE(boundary_cast.endpoint_hit) << "Probe generation must avoid visibility endpoints";
    ...
    if (!reference_visible) {
      ...
      EXPECT_TRUE(boundary_cast.positive_parameters.front() == reference_extent)
        << "candidate=" << CGAL::to_double(boundary_cast.positive_parameters.front())
...
void fillRectangle(
  GridMap& map, unsigned int min_x, unsigned int min_y, unsigned int max_x, unsigned int max_y) {
  for (unsigned int y = min_y; y < max_y; ++y)
    for (unsigned int x = min_x; x < max_x; ++x) map.data[y * map.width + x] = 1;
}
...
TEST(VisibilityReference, ObstacleVertexOpenFansMatchNaiveExactRayCasting) {
  ...
  Polymap rectangle_polymap = makeReadyPolymap(rectangle_map, 2, 6, 13, 10);
  ASSERT_TRUE(rectangle_polymap.isCDTReady());
  ASSERT_TRUE(rectangle_polymap.isValidTopology(rectangle_polymap.locateVertex(6, 4)));
  ...
  Polymap concave_polymap = makeReadyPolymap(concave_map, 3, 8, 11, 9);
  ASSERT_TRUE(concave_polymap.isCDTReady());
  ASSERT_TRUE(concave_polymap.isValidTopology(concave_polymap.locateVertex(9, 10)));
...
TEST(VisibilityReference, ClosedSameRayDiscontinuitiesKeepDirectedOrder) {
  ...
  VisibilityRegion visibility_region;
  std::string error;
  ASSERT_TRUE(polymap.getVisibilityRegion(2, 6, visibility_region, &error)) << error;

  const Point source(2, 6);
  const auto lower = std::find_if(
    visibility_region.begin(), visibility_region.end(), [](const BoundaryEndpoint& endpoint) {
  ...
  EXPECT_TRUE(exact_geometry::isSameDirectedRay(source, lower_far, lower_near));
  EXPECT_TRUE(squaredDistance(source, lower_far) > squaredDistance(source, lower_near));
  ...
  EXPECT_TRUE(exact_geometry::isSameDirectedRay(source, upper_near, upper_far));
  EXPECT_TRUE(squaredDistance(source, upper_near) < squaredDistance(source, upper_far));
}
