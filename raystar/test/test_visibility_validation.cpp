...
VisibilityGeometryContext closedRootContext(const Point& source)
{
  return VisibilityGeometryContext{
    source,
    VisibilityBoundaryMode::closed_cycle,
    std::nullopt,
    std::nullopt};
}

VisibilityGeometryContext openSectorContext(
  const Point& source, const Point& start, const Point& end)
{
  return VisibilityGeometryContext{
    source,
    VisibilityBoundaryMode::open_sector,
    start,
    end};
}
...
void expectRejected(
  const VisibilityRegion& boundary,
  const VisibilityGeometryContext& context,
  const std::string& expected_error_fragment)
{
  std::string error = "stale error";
  EXPECT_FALSE(raystar::detail::validateVisibilityGeometry(
    boundary, context, &error));
  ASSERT_FALSE(error.empty());
  EXPECT_NE(lowerCase(error).find(expected_error_fragment), std::string::npos)
    << error;
}
...
TEST(VisibilityGeometryValidation, ClosedRootAllowsAdjacentRepeatedEndpoint)
{
  ...
  EXPECT_TRUE(raystar::detail::validateVisibilityGeometry(
    boundary, closedRootContext(Point(0, 0)), &error)) << error;
...
TEST(VisibilityGeometryValidation, ClosedRootRejectsSourceEndpoint)
{
  ...
  expectRejected(
    boundary, closedRootContext(Point(0, 0)), "source");
}
...
TEST(VisibilityGeometryValidation, ClosedRootRejectsRayOrderRegression)
{
  ...
  expectRejected(
    boundary, closedRootContext(Point(0, 0)), "order");
}
...
TEST(VisibilityGeometryValidation, ClosedRootRejectsProperCrossing)
{
  ...
  expectRejected(
    boundary, closedRootContext(Point(0, -10)), "cross");
}
...
TEST(VisibilityGeometryValidation, OpenSectorAcceptsMatchingAnchors)
{
  ...
  EXPECT_TRUE(raystar::detail::validateVisibilityGeometry(
    boundary, openSectorContext(source, start, end), &error)) << error;
  EXPECT_TRUE(error.empty());
}
...
TEST(VisibilityGeometryValidation, OpenSectorRejectsSpokeCrossing)
{
  ...
  expectRejected(
    boundary, openSectorContext(source, start, end), "cross");
}
...
TEST(VisibilityGeometryValidation, OpenSectorRejectsAnchorMismatch)
{
  ...
  const std::vector<VisibilityRegion> mismatched_boundaries = {
    makeBoundary({{2, 0}, {4, 4}, {0, 4}}),
    makeBoundary({{4, 0}, {4, 4}, {0, 2}})};
...
TEST(VisibilityGeometryValidation, OpenSectorRejectsRayOutsideSector)
{
  ...
  const std::vector<VisibilityRegion> out_of_sector_boundaries = {
    makeBoundary({{4, 0}, {-1, 6}, {0, 4}}),
    makeBoundary({{4, 0}, {-1, -1}, {0, 4}})};
...
TEST(VisibilityGeometryValidation, OpenSectorRejectsRayOrderRegression)
{
  ...
  expectRejected(
    boundary, openSectorContext(source, start, end), "order");
}
...
TEST(VisibilityGeometryValidation, StopsDuringQuadraticCrossingCheck)
{
  ...
  for (size_t index = 0; index < endpoint_count; ++index)
    boundary.emplace_back(makeEndpoint(1, 0));
  ...
  const auto crossing_boundary = makeBoundary({
    {-4, 0},
    {4, 4},
    {-4, 4},
    {4, 0}});
