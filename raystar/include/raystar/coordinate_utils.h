...
{
  return std::isfinite(static_cast<double>(map.resolution)) &&
    map.resolution > 0.0f && std::isfinite(map.origin_x) &&
...
inline double canonicalizeWorldGridCoordinate(
  double world, double origin, double resolution, unsigned int extent,
  double converted)
{
  ...
  if (!std::isfinite(nearest) || nearest < 0.0 ||
      nearest > static_cast<double>(extent))
  {
    return converted;
  }
  ...
  const double upward_ulp = std::abs(
    std::nextafter(reconstructed, std::numeric_limits<double>::infinity()) -
    reconstructed);
  const double downward_ulp = std::abs(
    reconstructed - std::nextafter(
      reconstructed, -std::numeric_limits<double>::infinity()));
...
{
  if (!hasValidWorldTransform(map) || map.width == 0 || map.height == 0 ||
  ...
  if (!std::isfinite(dx) || !std::isfinite(dy) || dx < 0.0 || dy < 0.0 ||
      dx >= static_cast<double>(map.width) ||
      dy >= static_cast<double>(map.height))
  {
    return false;
  }
...
inline bool worldToContinuousMap(const GridMap& map, double wx, double wy,
                                 double& gx, double& gy,
                                 unsigned int& cell_x, unsigned int& cell_y)
{
  ...
  double converted_x =
    (wx - map.origin_x) / static_cast<double>(map.resolution);
  double converted_y =
    (wy - map.origin_y) / static_cast<double>(map.resolution);
  converted_x = canonicalizeWorldGridCoordinate(
    wx, map.origin_x, static_cast<double>(map.resolution), map.width,
    converted_x);
  converted_y = canonicalizeWorldGridCoordinate(
    wy, map.origin_y, static_cast<double>(map.resolution), map.height,
    converted_y);
  if (!std::isfinite(converted_x) || !std::isfinite(converted_y) ||
      converted_x < 0.0 || converted_y < 0.0 ||
      converted_x >= static_cast<double>(map.width) ||
  ...
  if (floored_x < 0.0 || floored_y < 0.0 ||
      floored_x >= static_cast<double>(map.width) ||
                                 ...
                                 ContinuousGridPoint& result)
{
  ...
  if (!worldToContinuousMap(
      map, wx, wy, candidate.x, candidate.y,
...
inline bool mapToWorld(const GridMap& map, unsigned int mx, unsigned int my,
                       double& wx, double& wy)
{
...
inline bool continuousMapToWorld(const GridMap& map, double gx, double gy,
                                 double& wx, double& wy)
{
  wx = std::numeric_limits<double>::quiet_NaN();
  wy = std::numeric_limits<double>::quiet_NaN();
  if (!hasValidWorldTransform(map) || !std::isfinite(gx) ||
      !std::isfinite(gy) || gx < 0.0 || gy < 0.0 ||
      gx >= static_cast<double>(map.width) ||
      gy >= static_cast<double>(map.height))
  {
    return false;
  }
...
inline bool worldToMapContinuous(const GridMap& map, double wx, double wy,
                                 double& gx, double& gy,
                                 unsigned int& cell_x, unsigned int& cell_y)
{
...
inline bool mapContinuousToWorld(const GridMap& map, double gx, double gy,
                                 double& wx, double& wy)
{
...
inline bool isAngleWithinCounterClockwiseSweep(double angle,
                                               ...
                                               double epsilon = 1e-7)
{
  if (!std::isfinite(angle) || !std::isfinite(start_angle) ||
      !std::isfinite(end_angle) || !std::isfinite(epsilon))
  {
    return false;
  }
  ...
  return relative_angle <= span + tolerance ||
    kTwoPi - relative_angle <= tolerance;
}
...
inline int orientation(std::pair<int, int> p, std::pair<int, int> q, std::pair<int, int> r)
{
  ...
  const SignedProduct first = product(
    difference(q.second, p.second), difference(r.first, q.first));
  const SignedProduct second = product(
    difference(q.first, p.first), difference(r.second, q.second));
  ...
  {
    const bool first_has_larger_magnitude =
      first.magnitude > second.magnitude;
    determinant_sign = first.negative ?
      (first_has_larger_magnitude ? -1 : 1) :
      (first_has_larger_magnitude ? 1 : -1);
  }

  if (determinant_sign == 0) return 0;
...
{
  return r.first >= std::min(p.first, q.first) &&
         r.first <= std::max(p.first, q.first) &&
         r.second >= std::min(p.second, q.second) &&
         r.second <= std::max(p.second, q.second);
}

inline bool segmentsIntersect(std::pair<int, int> p1, std::pair<int, int> p2,
                              std::pair<int, int> p3, std::pair<int, int> p4)
{
  int o1 = orientation(p1, p2, p3);  int o2 = orientation(p1, p2, p4);
...
inline OperationStatus newPathSelfCrosses(
  const std::vector<std::pair<int, int>>& path,
  std::pair<int, int> new_waypoint, bool& self_crosses,
