...
bool validateMapResourceBudget(
  ...
  std::string& error)
{
  ...
  {
    error = "Invalid map: data size is " + std::to_string(data_size) +
  ...
  if (cell_count > std::numeric_limits<size_t>::max() /
      kEstimatedPlannerMapBytesPerCell)
  {
  ...
  const size_t estimated_bytes =
    cell_count * kEstimatedPlannerMapBytesPerCell;
  if (estimated_bytes > limits.max_map_bytes)
  {
    error = "Invalid map: estimated planner map memory " +
      std::to_string(estimated_bytes) + " bytes exceeds max_map_bytes=" +
...
bool validateMapShapeAndAdmission(const GridMap& grid_map,
  const PlanningLimits& limits, size_t& width, size_t& height,
  size_t& cell_count, std::string& error)
{
  ...
  if (!validateMapResourceBudget(
      width, height, grid_map.data.size(), limits, estimate, error))
  {
    return false;
  }
...
bool validateNonMapPlanningLimits(
  const PlanningLimits& limits, std::string& error)
{
  const size_t max_int = static_cast<size_t>(std::numeric_limits<int>::max());
  if (limits.max_k <= 0)
  {
  ...
  {
    error = "Invalid planning limits: max_nodes must be between 1 and " +
  ...
  {
    error = "Invalid planning limits: max_debug_nodes must be between 0 and " +
      std::to_string(max_int);
    return false;
  }
...
bool validatePlanRequest(const GridMap& grid_map,
  int start_x, int start_y, int goal_x, int goal_y, int K,
  ...
  std::string& error)
{
  ...
  size_t height = 0;
  size_t cell_count = 0;
  if (!validateMapShapeAndAdmission(
      grid_map, limits, width, height, cell_count, error))
  {
    return false;
  }
  (void)cell_count;
  ...
  const auto is_in_bounds = [width, height](int x, int y) {
    return x >= 0 && y >= 0 &&
      static_cast<size_t>(x) < width && static_cast<size_t>(y) < height;
  };
  ...
  {
    error = "Invalid goal: coordinates (" + std::to_string(goal_x) + ", " +
      std::to_string(goal_y) + ") are outside map bounds";
    return false;
  }
...
std::string planningLimitMessage(
  PlanningLimitReached limit, const PlanningLimits& limits,
  size_t path_count)
{
  std::string message;
  if (limit == PlanningLimitReached::max_nodes)
  {
    message = "Planning stopped after reaching max_nodes=" +
      std::to_string(limits.max_nodes);
  }
  else if (limit == PlanningLimitReached::max_path_points)
  {
    message = "Planning stopped before exceeding max_path_points=" +
      std::to_string(limits.max_path_points);
  }
  ...
  if (path_count == 0)
  {
    message += limit == PlanningLimitReached::cancelled ?
      "; no path was found before cancellation" :
      "; no path was found before the limit";
  }
  else
  {
    message += "; returning " + std::to_string(path_count) +
      " path(s), but the search is incomplete";
  }
  return message;
}
...
VisibilityRegion adaptLegacyVisibilityRegion(const Polymap* pMap,
  ...
  bool& valid, std::string& error)
{
  ...
  for (size_t i = 0; i < positions.size(); ++i)
  {
    ...
    {
      valid = false;
      error = "Visibility region contains a vertex outside the representable int range "
        "at position " + std::to_string(i);
      return {};
    }
    endpoint.exact_position = exact_geometry::Point(
      endpoint.position.first, endpoint.position.second);
    ...
    else
    {
      ...
      {
        const int next_vertex = (legacy.second + 1) %
          static_cast<int>(obstacle.ordered_vertices_.size());
        endpoint.support = DirectedObstacleEdge{
          ObstacleVertexId{legacy.first, legacy.second},
...
bool validatePlanEndpointShape(const GridMap& grid_map,
  const PlanEndpoint& endpoint, const char* label, std::string& error)
{
  ...
  // actual input error.
  if (!std::isfinite(endpoint.position_.first) ||
      !std::isfinite(endpoint.position_.second))
  {
  ...
  if (cell.first < 0 || cell.second < 0 ||
      static_cast<size_t>(cell.first) >= grid_map.width ||
      static_cast<size_t>(cell.second) >= grid_map.height)
  {
    error = std::string("Invalid ") + label + ": cell coordinates (" +
  ...
  if (floor_x != static_cast<double>(cell.first) ||
      floor_y != static_cast<double>(cell.second))
  {
    error = std::string("Invalid ") + label +
      ": continuous position and grid cell disagree";
    return false;
  }
  const auto index = static_cast<size_t>(cell.second) * grid_map.width +
    static_cast<size_t>(cell.first);
  if (index >= grid_map.data.size() || grid_map.data[index] != 0)
  {
    error = std::string("Invalid ") + label +
      ": corresponding occupancy cell is occupied";
    return false;
  }
  return true;
}
...
bool validatePlanRequest(const GridMap& grid_map,
  const PlanEndpoint& start, const PlanEndpoint& goal, int K,
  const PlanningLimits& limits, std::string& error)
{
...
OperationStatus hybridPathSelfCrosses(
  const Point2d& start, const std::vector<std::pair<int, int>>& turns,
  const Point2d& candidate, bool& self_crosses, const StopToken& stop_token)
{
  self_crosses = false;
  if (stop_token.poll())
  ...
  if (points.size() >= 4)
  {
    const auto new_segment = exact_geometry::Kernel::Segment_2(
      points[points.size() - 2], points.back());
    for (size_t i = 0; i + 2 < points.size() - 1; ++i)
    {
      if (stop_token.poll())
        return OperationStatus::stopped;
      const auto old_segment = exact_geometry::Kernel::Segment_2(
        points[i], points[i + 1]);
...
OperationStatus generateChildrenFromSource(
  const Polymap* pMap, int owner_index, const Point2d& source,
  const exact_geometry::Point& exact_source, double start_angle,
  double end_angle, double gcost, const VisibilityRegion& visibility_region,
  std::vector<Child>& output, const StopToken& stop_token, std::string* error)
{
  output.clear();
  if (error)
    error->clear();
  ...
  const auto visibility_status = pMap->validateVisibilityRegion(
  ...
  for (size_t i = 0; i < visibility_region.size(); ++i)
  {
    ...
    theta_list[i] = atan2(position.second - source.second,
      position.first - source.first);
    theta_list[i] = start_angle +
      normalize_angle_positive(theta_list[i] - start_angle);
  }
        ...
        exact_source, exactPoint(current_endpoint), exactPoint(next_endpoint)))
    {
      ...
      if (!supports_consecutive)
      {
        ...
        const auto next_distance =
          (exactPoint(next_endpoint) - exact_source).squared_length();
  ...
  for (const auto i : valid_child_indices)
  {
    ...
    const std::pair<int, int> near_topology = {
      near_vertex->obstacle, near_vertex->vertex};
    ...
    if (is_a_left_gap[i])
    {
      ...
      const double contour_angle = atan2(
        next_obs->second - near_endpoint.position.second,
        next_obs->first - near_endpoint.position.first);
      child_end_angle =
        child_start_angle +
        normalize_angle_positive(contour_angle -
          child_start_angle);
    }
    else
    {
      ...
      const double contour_angle = atan2(
        prev_obs->second - near_endpoint.position.second,
        prev_obs->first - near_endpoint.position.first);
      child_start_angle = contour_angle;
      child_end_angle = contour_angle +
    ...
    canonical_near_endpoint.position = {
      static_cast<double>(canonical.first), static_cast<double>(canonical.second)};
    canonical_near_endpoint.exact_position =
      exact_geometry::Point(canonical.first, canonical.second);
    pending_children.push_back(PendingChild{
      std::move(canonical_near_endpoint), far_endpoint, is_a_left_gap[i],
      child_start_angle, child_end_angle});
  }
  ...
  for (size_t i = 0; i < pending_children.size(); ++i)
  {
    ...
    const double child_gcost = gcost + std::hypot(
      source.first - pending.near_endpoint.position.first,
      source.second - pending.near_endpoint.position.second);
    generated_children.emplace_back(
      owner_index, static_cast<int>(i), pending.near_endpoint,
      pending.far_endpoint, pending.is_left_gap,
      pending.start_angle, pending.end_angle, child_gcost);
  }
  if (stop_token.poll())
    return stopped();
...
static detail::VisibilityBoundaryMode visibilityBoundaryModeForNode(
  const Polymap& polymap, const Node& node)
{
  ...
  if (node.hasValidSeed() && polymap.isValidTopology(
      polymap.locateVertex(node.seed().first, node.seed().second)))
  {
    return detail::VisibilityBoundaryMode::open_sector;
  }
  return detail::VisibilityBoundaryMode::closed_cycle;
...
Child::Child(int nindex, int cindex, const BoundaryEndpoint& c_endpoint,
  ...
  : Nindex_(nindex), Cindex_(cindex), start_angle_(0), end_angle_(0),
    ...
    o_(o_endpoint.position), c_endpoint_(c_endpoint), o_endpoint_(o_endpoint),
...
Node::Node(const Polymap* pMap, int Nindex, double start_x, double start_y,
  ...
  : Node(pMap, Nindex, start_x, start_y, Gcost, Hcost,
      visibility_region, StopToken{})
{
}
...
Node::Node(const Polymap* pMap, int Nindex, double start_x, double start_y,
  ...
  const VisibilityRegion& visibility_region,
  const StopToken& stop_token)
  : Nindex_(Nindex), seed_({safeIntCast(start_x), safeIntCast(start_y)}),
    seed_is_valid_(isRepresentableAsInt(start_x) && isRepresentableAsInt(start_y)),
    start_angle_(0), end_angle_(kTwoPi),
    ...
    visibility_region_valid_(true)
{
  ...
  if (stop_token.poll() ||
      !canonicalizeExactObstacleVertices(
        pMap, visibility_region_, stop_token) ||
      !projectVisibilityRegion(
        visibility_region_, V_, topo_V_, stop_token))
  {
    stop_initialization();
    return;
  }
  ...
  {
    const auto status = pMap->validateVisibilityRegion(
      visibility_region_, stop_token, &visibility_region_error_);
...
Node::Node(const Polymap* pMap, int Nindex, double seed_x, double seed_y,
  ...
  const VisibilityRegion& visibility_region)
  : Node(pMap, Nindex, seed_x, seed_y, Gcost, Hcost, parent_index,
      visibility_region, StopToken{})
{
}
...
Node::Node(const Polymap* pMap, int Nindex, double seed_x, double seed_y,
  ...
  const VisibilityRegion& visibility_region,
  const StopToken& stop_token)
  : Nindex_(Nindex), seed_({safeIntCast(seed_x), safeIntCast(seed_y)}),
    seed_is_valid_(isRepresentableAsInt(seed_x) && isRepresentableAsInt(seed_y)),
    start_angle_(0), end_angle_(0),
    ...
    visibility_region_valid_(true)
{
  ...
  visibility_region_ = adaptLegacyVisibilityRegion(
    pMap, visibility_region, topo_V,
    visibility_region_valid_, visibility_region_error_);
  const StopToken no_stop;
  (void)projectVisibilityRegion(
    visibility_region_, V_, topo_V_, no_stop);
}
...
OperationStatus Node::setFullVisibilityRegion(
  const VisibilityRegion& visibility_region,
  const StopToken& stop_token)
{
  if (stop_token.poll())
...
OperationStatus Node::generateChild(
  ...
  std::string* error)
{
  ...
  if (!pMap)
  {
    if (error) *error = "Cannot generate children without a Polymap";
    return OperationStatus::failure;
  }
  if (!seed_is_valid_)
  {
    if (error) *error = "Node seed is outside the representable int range";
    return OperationStatus::failure;
  }
  if (!visibility_region_valid_)
  {
    if (error) *error = visibility_region_error_.empty() ?
      "Visibility region could not be adapted" : visibility_region_error_;
    return OperationStatus::failure;
  }
  const Point2d source = {
    static_cast<double>(seed_.first), static_cast<double>(seed_.second)};
  const auto status = generateChildrenFromSource(
    pMap, Nindex_, source, exact_geometry::Point(seed_.first, seed_.second),
    start_angle_, end_angle_, Gcost_, visibility_region_, C_, stop_token, error);
  return status;
}
...
OperationStatus RaystarCore::outlineMap(
  ...
  const StopToken& stop_token)
{
  ...
  return stop_token.poll() ?
    OperationStatus::stopped : OperationStatus::success;
}

OperationStatus RaystarCore::getScopedVisibilityRegion(
  ...
  const StopToken& stop_token, std::string& error)
{
  ...
  const auto& child = N_[static_cast<size_t>(parent_index)]
    .C_[static_cast<size_t>(child_index)];
  ...
  auto endpoint_status = theMap.validateBoundaryEndpoint(
    child.c_endpoint_, stop_token, &endpoint_error);
  ...
  const auto full_visibility_status = theMap.getVisibilityRegion(
    new_source_point.first, new_source_point.second, fullV,
    stop_token, &visibility_error);
  ...
  const std::pair<int, int> child_topology = {
    child_vertex->obstacle, child_vertex->vertex};
  ...
  if (child.is_a_left_gap_)
  {
    ...
    const auto& vertices = theMap.obstacles()[static_cast<size_t>(child_vertex->obstacle)]
      .ordered_vertices_;
    const std::pair<int, int> end_topology = {
      child_vertex->obstacle,
      (child_vertex->vertex + 1) % static_cast<int>(vertices.size())};
  ...
  endpoint_status = theMap.validateBoundaryEndpoint(
    start_obs, stop_token, &endpoint_error);
  if (endpoint_status == OperationStatus::stopped)
    return stopped();
  if (endpoint_status == OperationStatus::failure)
    return fail("Scoped visibility start boundary is invalid: " + endpoint_error);
  endpoint_status = theMap.validateBoundaryEndpoint(
    end_obs, stop_token, &endpoint_error);
  if (endpoint_status == OperationStatus::stopped)
    return stopped();
  if (endpoint_status == OperationStatus::failure)
  ...
  std::string validation_error;
  const auto metadata_status = theMap.validateVisibilityRegion(
    visibility_region, stop_token, &validation_error);
...
PlanResult RaystarCore::plan(const GridMap& grid_map,
  ...
  int K, bool allow_self_crossing, const PlanningLimits& limits)
{
  std::string legacy_error;
  if (!validatePlanRequest(
      grid_map, start_x, start_y, goal_x, goal_y, K, limits, legacy_error))
  {
  ...
  return plan(grid_map,
    PlanEndpoint(start_x, start_y,
      static_cast<double>(start_x), static_cast<double>(start_y)),
    PlanEndpoint(goal_x, goal_y,
      static_cast<double>(goal_x), static_cast<double>(goal_y)),
    K, allow_self_crossing, limits);
}
  ...
  int K, bool allow_self_crossing, const PlanningLimits& limits)
{
  ...
  return plan(grid_map, endpoint_from_point(start), endpoint_from_point(goal),
    K, allow_self_crossing, limits);
}

PlanResult RaystarCore::plan(const GridMap& grid_map,
  const PlanEndpoint& start, const PlanEndpoint& goal,
  int K, bool allow_self_crossing, const PlanningLimits& limits) try
{
  ...
  std::string validation_error;
  if (!validatePlanRequest(
      grid_map, start, goal, K, limits, validation_error))
  {
    result.outcome = PlanningOutcome::invalid_request;
  ...
  const size_t path_reserve = std::min(
    static_cast<size_t>(K), limits.max_path_points / 2);
  ...
  const bool timeout_enabled =
    limits.planning_timeout != std::chrono::milliseconds::max();
  ...
  const auto stop_before_planner = [&](std::shared_ptr<const Polymap> polymap) {
    ...
    result.message = planningLimitMessage(
      result.limit_reached, limits, result.path_solutions.size());
  ...
  if (stop_token.poll())
    return stop_before_planner(nullptr);
  if (outlineMap(
      work_map.data, work_map.width, work_map.height,
      stop_token) == OperationStatus::stopped)
  {
    return stop_before_planner(nullptr);
  }
  ...
  if (work_map.at(static_cast<unsigned int>(start_x),
                  static_cast<unsigned int>(start_y)) != 0)
  {
    result.outcome = PlanningOutcome::invalid_request;
    result.message = "Invalid start: corresponding cell is occupied or on the map boundary";
    return result;
  }
  if (work_map.at(static_cast<unsigned int>(goal_x),
                  static_cast<unsigned int>(goal_y)) != 0)
  {
    result.outcome = PlanningOutcome::invalid_request;
  ...
  auto map_build = Polymap::create(
    work_map, start_x, start_y, goal_x, goal_y,
    start.position_, goal.position_, stop_token, limits);
  ...
  if (map_build.status == PolymapCreateStatus::failure || !map_build)
  {
    ...
    if (map_build.error.rfind(invalid_start_prefix, 0) == 0)
    {
      ...
      result.message = "Invalid start: point must be a strict free-space interior; " +
        map_build.error.substr(
          std::string(invalid_start_prefix).size());
    }
    else if (map_build.error.rfind(invalid_goal_prefix, 0) == 0)
    {
      ...
      result.message = "Invalid goal: point must be a strict free-space interior; " +
        map_build.error.substr(
          std::string(invalid_goal_prefix).size());
    }
    else
    {
  ...
  auto theMap = std::make_shared<Polymap>(
    std::move(*map_build.value));
  ...
  const auto fail_planning = [&](const std::string& message,
    ...
    result.plan_time_ms = std::chrono::duration_cast<std::chrono::microseconds>(
      failure_time - planner_start_time).count() / 1000.0;
  ...
  std::string endpoint_error;
  const auto start_interior_status = theMap->validateFreeSpaceInterior(
    start.position_, stop_token, &endpoint_error);
  ...
  {
    return fail_planning("Invalid start: point must be a strict free-space interior; " +
      endpoint_error, PlanningOutcome::invalid_request);
  }
  const auto goal_interior_status = theMap->validateFreeSpaceInterior(
    goal.position_, stop_token, &endpoint_error);
  if (goal_interior_status == OperationStatus::stopped || stop_token.poll())
    return stop_before_planner(theMap);
  if (goal_interior_status == OperationStatus::failure)
  {
    return fail_planning("Invalid goal: point must be a strict free-space interior; " +
      endpoint_error, PlanningOutcome::invalid_request);
  }
  ...
  const auto stop_for_limit = [&](PlanningLimitReached limit) {
    ...
    result.message = planningLimitMessage(
      limit, limits, result.path_solutions.size());
    result.plan_time_ms = std::chrono::duration_cast<std::chrono::microseconds>(
      stop_time - planner_start_time).count() / 1000.0;
    result.polymap = theMap;
    return result;
  };

  const auto stop_for_request = [&]() {
    return stop_for_limit(requested_stop_reason);
  };
  ...
  queue.emplace_back(Candidate(-1, -1,
    std::hypot(start_gx - goal_gx, start_gy - goal_gy)));
  ...
  while (!queue.empty())
  {
    ...
    if (parent_index == -1)
    {
      ...
      const auto visibility_status = theMap->getRootVisibilityRegion(
        start_position, Vtemp, stop_token, &visibility_error);
      ...
      Node new_node(theMap.get(), 0,
        static_cast<double>(start_x), static_cast<double>(start_y), 0.0,
        best_candidate.Fcost_, Vtemp, stop_token);
      ...
      if (stop_token.poll())
        return stop_for_request();
      const auto full_visibility_status = new_node.setFullVisibilityRegion(
        Vtemp, stop_token);
      ...
      const auto child_status = generateChildrenFromSource(
        theMap.get(), new_node.Nindex_, start_position,
        exact_geometry::Point(start_gx, start_gy),
        new_node.start_angle_, new_node.end_angle_, new_node.Gcost_,
        new_node.visibility_region_, new_node.C_, stop_token, &child_error);
    ...
    else
    {
      ...
      {
        const auto crossing_status = hybridPathSelfCrosses(
          {start_gx, start_gy}, N_[parent_index].local_shortest_path_,
          {static_cast<double>(new_source_point.first),
           static_cast<double>(new_source_point.second)},
          path_self_crosses, stop_token);
      ...
      const auto visibility_status = getScopedVisibilityRegion(
        *theMap, best_candidate, Vtemp, fullVtemp,
        stop_token, visibility_error);
      if (visibility_status == OperationStatus::stopped || stop_token.poll())
        return stop_for_request();
      if (visibility_status == OperationStatus::failure)
      ...
      Node new_node(theMap.get(), new_node_index,
        new_source_point.first, new_source_point.second,
        N_[parent_index].C_[child_index].c_gcost_,
        N_[parent_index].C_[child_index].c_hcost_,
        parent_index, Vtemp, stop_token);
      if (stop_token.poll())
        return stop_for_request();
      const auto full_visibility_status = new_node.setFullVisibilityRegion(
        fullVtemp, stop_token);
      if (full_visibility_status == OperationStatus::stopped || stop_token.poll())
        return stop_for_request();
      if (full_visibility_status == OperationStatus::failure)
      ...
      new_node.local_shortest_path_.clear();
      new_node.local_shortest_path_.reserve(
        N_[parent_index].local_shortest_path_.size() + 1);
      ...
      std::string child_error;
      const auto child_status = new_node.generateChild(
        theMap.get(), stop_token, &child_error);
      if (child_status == OperationStatus::stopped || stop_token.poll())
        return stop_for_request();
      if (child_status == OperationStatus::failure)
    ...
    for (auto& child : pending_node->C_)
    {
      if (stop_token.poll())
        return stop_for_request();
      child.c_hcost_ = std::hypot(
        child.c_endpoint_.position.first - goal_gx,
    ...
    for (const auto& child : N_.back().C_)
    {
      ...
      queue.emplace_back(Candidate(
        child.Nindex_, child.Cindex_, child.c_gcost_ + child.c_hcost_));
      std::push_heap(queue.begin(), queue.end(), comp);
    }

    if (stop_token.poll())
      return stop_for_request();
    const auto point_location_mode = visibilityBoundaryModeForNode(
      *theMap, N_.back());
    ...
    if (b.first || b.second)
    {
      ...
      {
        const auto crossing_status = hybridPathSelfCrosses(
          {start_gx, start_gy}, N_.back().local_shortest_path_,
          {goal_gx, goal_gy}, goal_segment_crosses, stop_token);
        if (crossing_status == OperationStatus::stopped || stop_token.poll())
          return stop_for_request();
        if (crossing_status == OperationStatus::failure)
      ...
      {
        const size_t turning_point_count =
          N_.back().local_shortest_path_.size();
        ...
        if (turning_point_count > limits.max_path_points - 2 ||
            accumulated_path_points > limits.max_path_points -
        ...
        const double new_path_length = N_.back().Gcost_ +
          std::hypot(node_source.first - goal_gx,
                     node_source.second - goal_gy);

        PathSolution solution(
          {start_gx, start_gy}, N_.back().local_shortest_path_,
          {goal_gx, goal_gy}, new_path_length,
