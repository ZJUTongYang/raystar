...
bool continuousGridToWorld(const GridMap& grid_map, const Point2d& point, double& wx, double& wy) {
  ...
  const double converted_x =
    grid_map.origin_x + point.first * static_cast<double>(grid_map.resolution);
  const double converted_y =
    grid_map.origin_y + point.second * static_cast<double>(grid_map.resolution);
...
bool RaystarNode::occupancyGridToBinaryMap(const nav_msgs::msg::OccupancyGrid& grid,
  ...
  const double max_world_x =
    origin.position.x + static_cast<double>(width) * static_cast<double>(grid.info.resolution);
  const double max_world_y =
    origin.position.y + static_cast<double>(height) * static_cast<double>(grid.info.resolution);
  ...
  try {
    ...
    if (rejected_goal) {
      initializePlanningResponse(
        *result, rejected_goal->k, rejected_goal->map_id, rejected_goal->include_debug);
    } else {
...
void RaystarNode::executeAction(
  ...
  try {
    ...
    if (goal) {
      ...
      if (!resolveCachedMap(goal->map_id, cached_map, map_error)) {
        initializePlanningResponse(*result, goal->k, goal->map_id, goal->include_debug);
    ...
    try {
      const uint32_t requested_path_count = result ? result->result_info.requested_path_count : 0u;
      if (!result)
        result = std::make_shared<PlanAction::Result>();
      resetPlanningResponse(*result);
      result->result_info.status = PlanningResultInfo::STATUS_FAILED;
  ...
  if (explicit_cancel) {
    ...
    if (!goal_is_canceling()) {
      terminal_state = TerminalState::aborted;
      if (result) {
        const uint32_t requested_path_count = result->result_info.requested_path_count;
        resetPlanningResponse(*result);
        result->result_info.requested_path_count = requested_path_count;
        markFailed(*result);
        result->message =
...
void RaystarNode::executePlanning(const RequestT& request_value,
  ...
  initializePlanningResponse(*response, request->k, map_id, request->include_debug);
  ...
  case PlanningOutcome::complete:
    ...
    result_info.status = result.path_solutions.size() >= static_cast<size_t>(request->k)
  ...
  for (size_t solution_index = 0; solution_index < result.path_solutions.size(); ++solution_index) {
    ...
    if (point_count > configuration.planning.max_path_points - emitted_path_points) {
      omitted_path_count = result.path_solutions.size() - emitted_solution_indices.size();
      path_output_limits =
        static_cast<uint16_t>(path_output_limits | PlanningResultInfo::LIMIT_MAX_PATH_POINTS);
      appendResponseNotice(response->message,
    ...
    size_t path_bytes = 0;
    if (!estimatePathResponseBytes(point_count, grid.header.frame_id.size(), path_bytes) ||
  ...
  result_info.output_complete =
    omitted_path_count == 0 && response->path_results.size() == result.path_solutions.size();
  ...
  std::lock_guard<std::mutex> planner_lock(planner_cache_mutex_);
  clearVisualizationsLocked();
  initializePlanningResponse(response_value, request_value.k, map_id, request_value.include_debug);
  if (stop_requested && stop_requested()) {
