...
bool areStrictlyOpposite(
  CGAL::Orientation first, CGAL::Orientation second)
{
  return
    (first == CGAL::LEFT_TURN && second == CGAL::RIGHT_TURN) ||
    (first == CGAL::RIGHT_TURN && second == CGAL::LEFT_TURN);
}

bool hasProperInteriorCrossing(
  const ExactEdge& first, const ExactEdge& second)
{
  if (first.from == first.to || second.from == second.to)
    return false;

  const CGAL::Orientation first_from = CGAL::orientation(
    first.from, first.to, second.from);
  const CGAL::Orientation first_to = CGAL::orientation(
    first.from, first.to, second.to);
  const CGAL::Orientation second_from = CGAL::orientation(
    second.from, second.to, first.from);
  const CGAL::Orientation second_to = CGAL::orientation(
    second.from, second.to, first.to);

  return areStrictlyOpposite(first_from, first_to) &&
    areStrictlyOpposite(second_from, second_to);
}

bool areAdjacentCycleEdges(size_t first, size_t second, size_t edge_count)
{
  return (first + 1) % edge_count == second ||
  ...
  std::string* error)
{
  ...
  return validateVisibilityGeometry(
    visibility_region, context, never_stop, error) ==
  ...
  std::string* error)
{
  ...
  if (context.mode == VisibilityBoundaryMode::open_sector)
  {
    ...
    if (*context.start_anchor == context.source ||
        *context.end_anchor == context.source)
    {
  ...
  for (size_t index = 0; index < visibility_region.size(); ++index)
  {
    ...
    {
      return fail("Visibility endpoint " + std::to_string(index) +
        " coincides with source");
    }
  }

  if (context.mode == VisibilityBoundaryMode::open_sector &&
      (points.front() != *context.start_anchor ||
       points.back() != *context.end_anchor))
  {
  ...
  const size_t explicit_edge_count =
    context.mode == VisibilityBoundaryMode::closed_cycle ?
  ...
  for (size_t index = 0; index < explicit_edge_count; ++index)
  {
    ...
    const ExactPoint& to = context.mode == VisibilityBoundaryMode::closed_cycle ?
      points[(index + 1) % points.size()] : points[index + 1];
    if (from != to &&
        exact_geometry::Kernel::Segment_2(from, to).has_on(context.source))
    {
      return fail("Visibility boundary edge " + std::to_string(index) +
  ...
  for (size_t index = 0; index < cycle_vertices.size(); ++index)
  {
    ...
    cycle_edges.push_back(ExactEdge{
      cycle_vertices[index],
      cycle_vertices[(index + 1) % cycle_vertices.size()]});
  }
  ...
  if (context.mode == VisibilityBoundaryMode::closed_cycle)
  {
    for (size_t index = 1; index < points.size(); ++index)
    {
      ...
      if (exact_geometry::compareRaysLikeAtan2(
          context.source, points[index - 1], points[index]) == CGAL::LARGER)
      {
  ...
  for (size_t index = 0; index < points.size(); ++index)
  {
    ...
    if (index > 0 &&
        exact_geometry::compareRaysCounterClockwiseFrom(
          context.source, start, points[index - 1], points[index]) ==
          CGAL::LARGER)
    {
      return fail("Visibility ray order decreases between endpoints " +
        std::to_string(index - 1) + " and " + std::to_string(index));
