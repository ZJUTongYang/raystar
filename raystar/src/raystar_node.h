...
private:
  ...
  rclcpp_action::GoalResponse handleActionGoal(const rclcpp_action::GoalUUID& uuid,
  ...
  void executePlanning(const RequestT& request,
                       ResponseT& response,
  ...
  bool occupancyGridToBinaryMap(const nav_msgs::msg::OccupancyGrid& grid,
                                ...
                                GridMap& output,
                                std::string& error) const;
  bool loadRequestConfiguration(RequestConfiguration& configuration, std::string& error) const;

  void publishPolyObstacles(const Polymap& polymap,
                            const GridMap& grid_map,
                            const std::string& frame_id,
                            size_t max_marker_bytes);
  void publishCDT(const Polymap& polymap,
                  const GridMap& grid_map,
                  const std::string& frame_id,
                  size_t max_marker_bytes);
  void publishNonHomotopicPaths(const std::vector<PathSolution>& solutions,
                                const GridMap& grid_map,
                                const std::string& frame_id,
                                size_t max_path_points,
                                size_t max_marker_bytes);
  void publishDebugTree(const std::vector<raystar::Node>& nodes,
                        const GridMap& grid_map,
                        const std::string& frame_id,
                        size_t max_debug_nodes,
                        size_t max_marker_bytes);

  bool buildPathMsg(const PathSolution& solution,
                    const GridMap& grid_map,
                    const std::string& frame_id,
                    size_t max_path_points,
