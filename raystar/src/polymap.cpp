...
bool prepareGridStorage(const GridMap& grid_map, int& xsize, int& ysize,
  size_t& cell_count, std::string& error)
{
  ...
  {
    error = "Invalid map: data size is " + std::to_string(grid_map.data.size()) +
      ", expected " + std::to_string(cell_count);
    return false;
  }
...
bool validateCDTWithCGAL(
  const constrained_delaunay_triangulation::CDT& cdt)
{
...
struct UndirectedGridEdgeKeyHash
{
  size_t operator()(const UndirectedGridEdgeKey& edge) const noexcept
  {
    const uint64_t packed =
      (static_cast<uint64_t>(static_cast<uint32_t>(edge.low)) << 32) |
...
enum class ExactSegmentRelation
{
  disjoint,
  endpoint_touch,
  t_junction,
...
ExactSegmentRelationResult classifyExactSegments(
  const exact_geometry::Point& first_from,
  ...
  const exact_geometry::Point& second_to)
{
  const auto first_segment =
    exact_geometry::Kernel::Segment_2(first_from, first_to);
  const auto second_segment =
    exact_geometry::Kernel::Segment_2(second_from, second_to);

  const CGAL::Orientation second_from_side =
    CGAL::orientation(first_from, first_to, second_from);
  const CGAL::Orientation second_to_side =
    CGAL::orientation(first_from, first_to, second_to);
  const CGAL::Orientation first_from_side =
    CGAL::orientation(second_from, second_to, first_from);
  const CGAL::Orientation first_to_side =
    CGAL::orientation(second_from, second_to, first_to);
...
std::optional<detail::VisibilityGeometryContext>
makeVisibilityGeometryContext(
  const Polymap& polymap, const Point2d& source,
  ...
  const Polymap& polymap, int source_x, int source_y, std::string& error)
{
  ...
  return makeVisibilityGeometryContext(
    polymap, Point2d{static_cast<double>(source_x), static_cast<double>(source_y)},
    exact_geometry::Point(source_x, source_y),
    Polymap::isNoTopology(source_topology) ?
    std::optional<std::pair<int, int>>() :
    std::optional<std::pair<int, int>>(source_topology), error);
}
  ...
  std::string& error)
{
  ...
  detail::VisibilityGeometryContext context{
    exact_source,
    detail::VisibilityBoundaryMode::closed_cycle,
    std::nullopt,
    std::nullopt};
...
enum class ExactContourPointLocation
{
  outside,
  boundary,
  inside
  ...
  const StopToken& stop_token)
{
  ...
  for (size_t index = 0; index < vertices.size(); ++index)
  {
    ...
    const exact_geometry::Point from(
      from_coordinate.first, from_coordinate.second);
    const exact_geometry::Point to(
      to_coordinate.first, to_coordinate.second);
  ...
  return winding_number == 0 ?
    ExactContourPointLocation::outside : ExactContourPointLocation::inside;
}
...
// actually present.
bool hasClockwiseOuterContour(
  const std::vector<Obs>& obstacles, const StopToken& stop_token)
{
  for (const auto& obstacle : obstacles)
  {
    ...
    for (size_t index = 0; index < obstacle.ordered_vertices_.size(); ++index)
    {
      ...
      const auto& to = obstacle.ordered_vertices_[(index + 1) %
        obstacle.ordered_vertices_.size()];
      twice_area += exact_geometry::FT(from.first) * to.second -
        exact_geometry::FT(to.first) * from.second;
    }
...
static std::optional<exact_geometry::Point> findIntersection(
  const exact_geometry::Point& s, const exact_geometry::Point& g,
  const exact_geometry::Point& p, const exact_geometry::Point& limit)
{
  ...
  VisibilityRegion& visibility_region)
{
  ...
  return calculateVisibilityRegionImpl(
    round_x, round_y, visibility_region, no_stop);
}

OperationStatus Polymap::calculateVisibilityRegion(
  int round_x, int round_y, VisibilityRegion& visibility_region,
  const StopToken& stop_token)
{
  ...
  if (calculateVisibilityRegionImpl(
      round_x, round_y, visibility_region, stop_token))
  {
    if (stop_token.poll())
    {
      visibility_region.clear();
  ...
  return stop_token.poll() ?
    OperationStatus::stopped : OperationStatus::failure;
}

bool Polymap::calculateVisibilityRegionImpl(int round_x, int round_y,
  VisibilityRegion& visibility_region, const StopToken& stop_token)
{
  ...
  const std::optional<std::pair<int, int>> obstacle_vertex =
    ...
    std::optional<std::pair<int, int>>(topology) :
    std::nullopt;
  return calculateVisibilityRegionImpl(
    Point2d{static_cast<double>(round_x), static_cast<double>(round_y)},
    exact_geometry::Point(round_x, round_y), obstacle_vertex,
    visibility_region, stop_token);
}
  ...
  VisibilityRegion& visibility_region, const StopToken& stop_token)
{
  ...
  const size_t expected_registry_size =
    static_cast<size_t>(xsize_) * static_cast<size_t>(ysize_);
  ...
  const auto make_final_boundary = [](const BoundaryEndpoint& prev,
  ...
  const auto make_obstacle_boundary = [](const BoundaryEndpoint& prev,
      const BoundaryEndpoint& next, const DirectedObstacleEdge& support) {
    BungiuEdge edge;
    edge.prev = prev;
    edge.next = next;
    edge.is_bd = true;
  ...
  const auto make_portal = [](const BoundaryEndpoint& prev,
      const BoundaryEndpoint& next, double limit_prev, double limit_next,
  ...
  const auto set_final_boundary = [](BungiuEdge& edge,
  ...
  const auto make_final_or_supported_boundary = [&](const BoundaryEndpoint& prev,
      const BoundaryEndpoint& next,
      const std::optional<DirectedObstacleEdge>& possible_support) {
    if (possible_support &&
        pointLiesOnObstacleEdge(prev, *possible_support) &&
  ...
  const auto set_final_or_supported_boundary = [&](BungiuEdge& edge,
      const BoundaryEndpoint& prev, const BoundaryEndpoint& next,
      const std::optional<DirectedObstacleEdge>& possible_support) {
  ...
  const auto validate_bungiu_edge = [&](const BungiuEdge& edge) {
    ...
    {
      if (!edge.is_bd ||
          !pointLiesOnObstacleEdge(edge.prev, *edge.supporting_segment) ||
    ...
    {
      if (edge.supporting_segment ||
          !validateBoundaryEndpointImpl(
            edge.limit_prev_anchor, stop_token, nullptr) ||
          !validateBoundaryEndpointImpl(
            edge.limit_next_anchor, stop_token, nullptr) ||
          !exact_geometry::isPortalSpanAtMostPi(
            exact_source, exactPoint(edge.limit_prev_anchor),
            exactPoint(edge.limit_next_anchor)))
      {
        return false;
      }
    }
  ...
  if (obstacle_vertex_source)
  {
    ...
    const auto source_coordinate = obs_[static_cast<size_t>(curr.first)]
      .ordered_vertices_[static_cast<size_t>(curr.second)];
    ...
    if (exact_source != exact_geometry::Point(
          source_coordinate.first, source_coordinate.second))
      return false;
    ...
    while (!(the_vertex.first == next.first && the_vertex.second == next.second))
    {
      ...
      bd.back().limit_prev = atan2(bd.back().prev.position.second - source_y, bd.back().prev.position.first - source_x);
      bd.back().limit_next = atan2(bd.back().next.position.second - source_y, bd.back().next.position.first - source_x);
      ...
      bd.back().supporting_segment = obstacleEdgeBetween(
        bd.back().next.position, bd.back().prev.position);
  ...
  while (1)
  {
    ...
    if (loc < 0) { iter->is_bd = true; ++iter; continue; }
    int i;
    // Find the third vertex of the triangle (not prev_pos, not next_pos).
    for (i = 0; i < 3; ++i) {
      if (!(facets_[loc][i].first == iter->prev.position.first && facets_[loc][i].second == iter->prev.position.second) &&
        !(facets_[loc][i].first == iter->next.position.first && facets_[loc][i].second == iter->next.position.second))
        break;
    }
    if (i == 3)
      return false;
    ...
    theta = iter->limit_prev + normalize_angle(
      atan2(facets_[loc][i].second - source_y, facets_[loc][i].first - source_x) - iter->limit_prev);
    ray_position = exact_geometry::classifyPortalRay(
      exact_source, exactPoint(iter->limit_prev_anchor),
      exactPoint(iter->limit_next_anchor), exactPoint(V));
    ...
    if (ray_position == exact_geometry::PortalRayPosition::before_lower)
    {
      ...
      if (blocker)
      {
        ...
        const auto endpoint_prev_position = findIntersection(
          exactPoint(V), exactPoint(iter->next), exact_source,
        ...
        bd.insert(iter, make_final_or_supported_boundary(
          iter->limit_prev_anchor, endpoint_prev, blocker));
        ...
        if (exactPoint(iter->next) != exactPoint(endpoint_next))
        {
          ...
          set_final_or_supported_boundary(
            *iter, endpoint_next, iter->limit_next_anchor, blocker);
          iter++;
        } else {
    ...
    else if (ray_position == exact_geometry::PortalRayPosition::equal_lower)
    {
      ...
      bd.insert(iter, make_final_or_supported_boundary(
        iter->limit_prev_anchor, V, blocker));
      if (blocker)
      {
    ...
    else if (ray_position == exact_geometry::PortalRayPosition::inside)
    {
      ...
      if (prev_blocker)
      {
        ...
        const auto endpoint_prev_position = findIntersection(
          exactPoint(iter->prev), exactPoint(V), exact_source,
          exactPoint(iter->limit_prev_anchor));
        ...
        if (exactPoint(endpoint_prev) != exactPoint(iter->limit_prev_anchor))
        {
          ...
          bd.insert(iter, make_final_or_supported_boundary(
            iter->limit_prev_anchor, endpoint_prev, prev_blocker));
        }
      ...
      else
      {
        ...
        bd.insert(iter, make_portal(iter->prev, V,
          iter->limit_prev, theta, iter->limit_prev_anchor, V));
        to_minus = true;
      }
      ...
      if (next_blocker)
      {
        ...
        if (exactPoint(iter->limit_next_anchor) != exactPoint(endpoint_next))
        {
          ...
          if (to_minus) { iter = std::prev(ni); } else { iter++; }
        }
        else
        {
          // Far-endpoint coincides with limit_next_pos: erase iter
          bd.erase(iter++);
          if (to_minus) { iter = std::prev(ni); }
        }
      }
      else
      {
    ...
    else if (ray_position == exact_geometry::PortalRayPosition::equal_upper)
    {
      ...
      if (prev_blocker)
      {
        // Obstacle edge between V and prev_pos.
        const auto endpoint_prev_position = findIntersection(
          exactPoint(V), exactPoint(iter->prev), exact_source,
          exactPoint(iter->limit_prev_anchor));
        if (!endpoint_prev_position)
          return false;
        const auto endpoint_prev = makeIntersectionEndpoint(*endpoint_prev_position, *prev_blocker);
      ...
      else if (next_blocker)
      {
        ...
        auto ni = bd.insert(iter, make_portal(iter->prev, V,
          iter->limit_prev, iter->limit_next,
          iter->limit_prev_anchor, iter->limit_next_anchor));
        bd.erase(iter);
        iter = ni;
      }
      else
  ...
  for (const auto& edge : bd)
  {
    ...
    if (!validate_bungiu_edge(edge))
      return false;
    if (exactPoint(edge.prev) == exact_source ||
        exactPoint(edge.next) == exact_source)
    {
      return false;
    }
  }
  ...
  {
    const exact_geometry::Point reference(
      open_region_start_vertex.first, open_region_start_vertex.second);
    if (stop_token.poll())
      return false;
    std::stable_sort(sorted_edges.begin(), sorted_edges.end(),
      [&exact_source, &reference](const auto& first, const auto& second) {
        return exact_geometry::compareRaysCounterClockwiseFrom(
          exact_source, reference, exactPoint(first.edge->prev),
  ...
  {
    const auto compare_closed_rays = [&exact_source](
        const auto& first, const auto& second) {
      return exact_geometry::compareRaysLikeAtan2(
        exact_source, exactPoint(first.edge->prev),
        exactPoint(second.edge->prev));
    };
    if (stop_token.poll())
      return false;
    std::stable_sort(sorted_edges.begin(), sorted_edges.end(),
    ...
    while (group_begin < sorted_edges.size())
    {
      ...
      while (group_end < sorted_edges.size() &&
          compare_closed_rays(
            sorted_edges[group_begin], sorted_edges[group_end]) == CGAL::EQUAL)
      {
        if (stop_token.poll())
          return false;
        ++group_end;
      ...
      if (group_end - group_begin > 1)
      {
        ...
        const auto rotation_satisfies_radial_edges = [&](size_t rotation) {
          ...
          for (size_t local_from = 0; local_from < group_size; ++local_from)
          {
            ...
            if (from_point == next_point ||
                !exact_geometry::isSameDirectedRay(
                  exact_source, from_point, next_point))
            {
              continue;
            }
        ...
        if (!rotation_satisfies_radial_edges(0))
        {
          ...
          std::rotate(
            ...
            sorted_edges.begin() + static_cast<std::ptrdiff_t>(
              group_begin + valid_rotation),
  ...
  if (!validateVisibilityRegionImpl(
      visibility_region, stop_token, &validation_error))
  {
    visibility_region.clear();
    return false;
  }
  ...
  if (!context || detail::validateVisibilityGeometry(
      visibility_region, *context, stop_token, &validation_error) !=
...
bool Polymap::calculateVisibilityRegion(int x, int y,
...
BoundaryEndpoint Polymap::makeEndpoint(const Point2d& position) const
{
  ...
  if (isValidTopology(topology))
  {
    ...
    endpoint.position = {
      static_cast<double>(vertex.first), static_cast<double>(vertex.second)};
...
BoundaryEndpoint Polymap::makeEndpoint(const std::pair<int, int>& position) const
{
  BoundaryEndpoint endpoint;
  endpoint.position = {
    static_cast<double>(position.first), static_cast<double>(position.second)};
  endpoint.exact_position = exact_geometry::Point(position.first, position.second);
  const auto topology = locateVertex(position.first, position.second);
  if (isValidTopology(topology))
    endpoint.support = fromLegacyTopology(topology);
...
BoundaryEndpoint Polymap::makeIntersectionEndpoint(const exact_geometry::Point& position,
...
std::optional<DirectedObstacleEdge> Polymap::obstacleEdgeBetween(
  const Point2d& from, const Point2d& to) const
{
  ...
  if (!areConsecutive(from_topology, to_topology))
    return std::nullopt;
  return DirectedObstacleEdge{
    fromLegacyTopology(from_topology), fromLegacyTopology(to_topology)};
}
...
bool Polymap::validateBoundaryEndpoint(const BoundaryEndpoint& endpoint,
...
OperationStatus Polymap::validateBoundaryEndpoint(
...
bool Polymap::validateBoundaryEndpointImpl(
  const BoundaryEndpoint& endpoint, const StopToken& stop_token,
  std::string* error) const
{
  if (error)
  ...
  if (!std::isfinite(endpoint.position.first) ||
      !std::isfinite(endpoint.position.second))
  {
    return fail("Boundary endpoint position is not finite");
  }
  if (endpoint.exact_position &&
      endpoint.position != projectExactPoint(*endpoint.exact_position))
  {
    ...
    for (size_t obstacle_index = 0; obstacle_index < obs_.size(); ++obstacle_index)
    {
      ...
      for (size_t vertex_index = 0; vertex_index < vertices.size(); ++vertex_index)
      {
        ...
        const DirectedObstacleEdge edge{
          ObstacleVertexId{static_cast<int>(obstacle_index),
...
OperationStatus Polymap::validateVisibilityRegion(
...
bool Polymap::validateVisibilityRegionImpl(
  const VisibilityRegion& visibility_region, const StopToken& stop_token,
  std::string* error) const
{
  if (error)
  ...
  for (size_t i = 0; i < visibility_region.size(); ++i)
  {
    ...
    if (!validateBoundaryEndpointImpl(
        visibility_region[i], stop_token, &endpoint_error))
    {
      if (error)
      {
        *error = "Visibility endpoint " + std::to_string(i) + " is invalid: " +
          endpoint_error;
      }
      return false;
    }
...
OperationStatus Polymap::boundarySupportsConsecutive(
  const BoundaryEndpoint& prev, const BoundaryEndpoint& next,
  bool& supports, const StopToken& stop_token) const
{
  ...
  if (stop_token.poll())
    return OperationStatus::stopped;
  if (boundarySupportsConsecutiveImpl(
      prev, next, supports, stop_token))
  {
    if (stop_token.poll())
    {
      supports = false;
...
bool Polymap::boundarySupportsConsecutiveImpl(
  const BoundaryEndpoint& prev, const BoundaryEndpoint& next,
  bool& supports, const StopToken& stop_token) const
{
  supports = false;
  ...
  {
    supports = areConsecutive(
      toLegacyTopology(*prev_vertex), toLegacyTopology(*next_vertex));
    return true;
  }
...
bool Polymap::isInTri(int x1, int y1, int x2, int y2, int x3, int y3, double x, double y)
{
  const exact_geometry::Kernel::Triangle_2 triangle(
    exact_geometry::Point(x1, y1),
    exact_geometry::Point(x2, y2),
    exact_geometry::Point(x3, y3));
...
void Polymap::clearStoppedConstructionState()
{
  ...
  obs_.clear();
  std::fill(
    vertices_location_x_flat_.begin(), vertices_location_x_flat_.end(), -1);
  std::fill(
    vertices_location_y_flat_.begin(), vertices_location_y_flat_.end(), -1);
  clearCGALRelatedState();
}

bool Polymap::constructCGALRelated(
...
OperationStatus Polymap::constructCGALRelated(
  CdtValidator validator, std::string& error,
  const StopToken& stop_token)
{
  error.clear();
...
bool Polymap::constructCGALRelatedImpl(
  CdtValidator validator, std::string& error,
  const StopToken& stop_token)
{
  ...
  try
  {
    ...
    const int candidate_vertex_count =
      static_cast<int>(candidate_cdt.number_of_vertices());
    const long long map_area =
      static_cast<long long>(xsize_) * static_cast<long long>(ysize_);
    ...
    int count = 0;
    for (auto fit = candidate_cdt.finite_faces_begin();
      fit != candidate_cdt.finite_faces_end(); ++fit)
    {
      if (stop_token.poll())
        return false;
      candidate_facets.emplace_back(std::vector<std::pair<int, int>>{
        {static_cast<int>(fit->vertex(0)->point().x()),
  ...
  {
    return fail(
      "Obstacle constraints intersect, overlap, or form an unsupported T-junction");
  }
  catch (const CGAL::Failure_exception& exception)
  {
    return fail(
      "CGAL rejected the constrained triangulation: " +
      std::string(exception.what()));
  }
  catch (const std::exception& exception)
  {
    return fail(
      "Unexpected error while constructing the CDT: " +
      std::string(exception.what()));
  }
  catch (...)
  {
...
bool Polymap::getPolyObstacles(int start_x, int start_y, int goal_x, int goal_y)
{
  ...
  return getPolyObstacles(
    start_x, start_y, goal_x, goal_y, no_stop) ==
    OperationStatus::success;
}
  ...
  const StopToken& stop_token)
{
  ...
  if (getPolyObstaclesImpl(
      start_x, start_y, goal_x, goal_y, stop_token))
  {
  ...
  const StopToken& stop_token)
{
  ...
  std::unordered_map<
    UndirectedGridEdgeKey,
    DirectedGridEdge,
    UndirectedGridEdgeKeyHash> edges;
  const auto add_directed_edge = [&](int from, int to) {
    edges.emplace(
      makeUndirectedGridEdgeKey(from, to),
      DirectedGridEdge{from, to});
  };
  std::stack<int> Q;
  ...
  while (!edges.empty())
  {
    ...
    for (auto it = candidate_obstacles.back().ordered_vertices_.begin();
      it != candidate_obstacles.back().ordered_vertices_.end(); ++it)
    {
      if (stop_token.poll())
        return false;
...
bool Polymap::getVisibilityRegion(int start_x, int start_y,
  VisibilityRegion& visibility_region)
{
...
{
  const StopToken no_stop;
  return getVisibilityRegionImpl(
    start_x, start_y, visibility_region, no_stop, error);
}

OperationStatus Polymap::getVisibilityRegion(
  ...
  const StopToken& stop_token, std::string* error)
{
  ...
  if (stop_token.poll())
    return OperationStatus::stopped;
  if (getVisibilityRegionImpl(
      start_x, start_y, visibility_region, stop_token, error))
  {
    return OperationStatus::success;
  }
  visibility_region.clear();
...
bool Polymap::getVisibilityRegionImpl(
  int start_x, int start_y, VisibilityRegion& visibility_region,
  const StopToken& stop_token, std::string* error)
{
  visibility_region.clear();
  ...
  const auto validate_for_source = [&](const VisibilityRegion& region,
      std::string& validation_error) {
    if (!validateVisibilityRegionImpl(
        region, stop_token, &validation_error))
      return false;

    std::string context_error;
    const auto context = makeVisibilityGeometryContext(
      *this, start_x, start_y, context_error);
    if (!context)
    {
      validation_error = std::move(context_error);
      return false;
    }
    return detail::validateVisibilityGeometry(
      region, *context, stop_token, &validation_error) ==
      OperationStatus::success;
  };
  ...
  if (!calculateVisibilityRegionImpl(
      start_x, start_y, entry.vertices, stop_token))
...
bool Polymap::getRootVisibilityRegion(
...
OperationStatus Polymap::getRootVisibilityRegion(
  const Point2d& source, VisibilityRegion& visibility_region,
  const StopToken& stop_token, std::string* error)
{
  visibility_region.clear();
  ...
  std::string validation_error;
  const OperationStatus interior_status = validateFreeSpaceInterior(
    source, stop_token, &validation_error);
  ...
  if (!solution_exist_ || !cdt_ready_)
  {
    if (error)
    {
      *error = "Root visibility region is unavailable because map geometry "
        "construction failed";
      if (!construction_error_.empty())
...
bool Polymap::validateFreeSpaceInterior(
  const Point2d& point, std::string* error) const
{
  const StopToken no_stop;
  return validateFreeSpaceInterior(point, no_stop, error) ==
    OperationStatus::success;
}

OperationStatus Polymap::validateFreeSpaceInterior(
...
bool Polymap::validateFreeSpaceInteriorImpl(
  const Point2d& point, const StopToken& stop_token,
  std::string* error) const
{
  if (error)
  ...
  for (size_t obstacle_index = 0; obstacle_index < obs_.size(); ++obstacle_index)
  {
    ...
    else if (!found_clockwise_outer &&
        (obstacle_index == 0 || twice_area > outer_area))
    {
      outer_index = obstacle_index;
      outer_area = twice_area;
    }
  ...
  for (size_t obstacle_index = 0; obstacle_index < obs_.size(); ++obstacle_index)
  {
    if (stop_token.poll())
      return false;
    const auto location = locateExactPointInContour(
      obs_[obstacle_index].ordered_vertices_, exact_point, stop_token);
    if (stop_token.poll())
      return false;
  ...
  const auto outer_location = locateExactPointInContour(
    obs_[outer_index].ordered_vertices_, exact_point, stop_token);
  if (stop_token.poll())
    return false;
...
std::pair<int, int> Polymap::locateVertex(int x, int y) const
{
  ...
  const size_t idx = static_cast<size_t>(y) * static_cast<size_t>(xsize_) +
...
std::pair<int, int> Polymap::locateVertex(double x, double y) const
{
  ...
  if (std::abs(x - rounded_x) > integer_tolerance ||
      std::abs(y - rounded_y) > integer_tolerance ||
      rounded_x < 0.0 || rounded_y < 0.0 ||
      rounded_x >= static_cast<double>(xsize_) ||
...
bool Polymap::validateObstacleTopology(
  std::string& error, bool validate_edge_relations) const
{
  const StopToken no_stop;
  return validateObstacleTopologyImpl(
    error, validate_edge_relations, no_stop);
}

OperationStatus Polymap::validateObstacleTopology(
  ...
  const StopToken& stop_token) const
{
  ...
  if (stop_token.poll())
    return OperationStatus::stopped;
  if (validateObstacleTopologyImpl(
      error, validate_edge_relations, stop_token))
  {
    if (stop_token.poll())
    {
      error.clear();
...
bool Polymap::validateObstacleTopologyImpl(
  std::string& error, bool validate_edge_relations,
  const StopToken& stop_token) const
{
  error.clear();
  ...
  for (size_t obstacle_index = 0; obstacle_index < obs_.size(); ++obstacle_index)
  {
    ...
    if (vertices.size() < 3)
    {
      return fail("obstacle " + std::to_string(obstacle_index) +
    ...
    for (size_t vertex_index = 0; vertex_index < vertices.size(); ++vertex_index)
    {
      ...
      if (vertex.first < 0 || vertex.second < 0 ||
          vertex.first >= xsize_ || vertex.second >= ysize_)
      {
        return fail("obstacle " + std::to_string(obstacle_index) +
          " vertex " + std::to_string(vertex_index) + " at (" +
      ...
      const ObstacleVertexId current_owner{
        static_cast<int>(obstacle_index), static_cast<int>(vertex_index)};
      ...
      if (!inserted.second)
      {
        ...
        {
          return fail("obstacles " + std::to_string(previous_owner.obstacle) +
            " and " + std::to_string(current_owner.obstacle) +
            " share vertex (" + std::to_string(vertex.first) + ", " +
            std::to_string(vertex.second) +
            "); zero-clearance obstacle contacts are not supported");
        }
        return fail("obstacle " + std::to_string(current_owner.obstacle) +
          " repeats vertex (" + std::to_string(vertex.first) + ", " +
          std::to_string(vertex.second) + ") at indices " +
          std::to_string(previous_owner.vertex) + " and " +
      ...
      {
        return fail("obstacle " + std::to_string(obstacle_index) +
          " edge " + std::to_string(vertex_index) +
          " has zero length at (" + std::to_string(vertex.first) + ", " +
          std::to_string(vertex.second) + ")");
      }

      twice_signed_area +=
        exact_geometry::FT(vertex.first) * exact_geometry::FT(next.second) -
        exact_geometry::FT(next.first) * exact_geometry::FT(vertex.second);
      if (validate_edge_relations)
      {
        edges.push_back(ObstacleEdgeRef{
          obstacle_index,
          vertex_index,
          vertices.size(),
          vertex,
  ...
  for (size_t first_index = 0; first_index < edges.size(); ++first_index)
  {
         ...
         second_index < edges.size(); ++second_index)
    {
      ...
      const bool first_precedes_second = same_obstacle &&
        (first.edge + 1) % first.obstacle_size == second.edge;
      const bool second_precedes_first = same_obstacle &&
        (second.edge + 1) % second.obstacle_size == first.edge;
      if (first_precedes_second || second_precedes_first)
      {
        ...
        if (relation.relation == ExactSegmentRelation::endpoint_touch &&
            relation.contact && *relation.contact == expected_contact)
        {
          continue;
        }
      }
      ...
      message << "obstacle " << first.obstacle << " edge " << first.edge
              << " [(" << first.from_coordinate.first << ", "
              << first.from_coordinate.second << ") -> ("
              << first.to_coordinate.first << ", "
              << first.to_coordinate.second << ")] and obstacle "
              << second.obstacle << " edge " << second.edge << " [("
              << second.from_coordinate.first << ", "
              << second.from_coordinate.second << ") -> ("
              << second.to_coordinate.first << ", "
              << second.to_coordinate.second << ")] "
...
bool Polymap::simplificationChordIsTopologicallySafe(
  size_t obstacle_index, size_t current_index) const
{
  const StopToken no_stop;
  return simplificationChordIsTopologicallySafeImpl(
    obstacle_index, current_index, no_stop);
}

OperationStatus Polymap::simplificationChordIsTopologicallySafe(
  ...
  const StopToken& stop_token) const
{
  ...
  if (stop_token.poll())
    return OperationStatus::stopped;
  safe = simplificationChordIsTopologicallySafeImpl(
    obstacle_index, current_index, stop_token);
  if (stop_token.poll())
  {
    safe = false;
    return OperationStatus::stopped;
...
bool Polymap::simplificationChordIsTopologicallySafeImpl(
  ...
  const StopToken& stop_token) const
{
  ...
  const size_t previous_index =
    (current_index + vertices.size() - 1) % vertices.size();
  ...
  const exact_geometry::Point previous(
  ...
  const size_t edge_before_previous =
    (previous_index + vertices.size() - 1) % vertices.size();
  ...
  for (size_t other_obstacle = 0; other_obstacle < obs_.size(); ++other_obstacle)
  {
    ...
    for (size_t edge_index = 0; edge_index < other_vertices.size(); ++edge_index)
    {
      ...
      const auto& edge_to_coordinate =
        other_vertices[(edge_index + 1) % other_vertices.size()];
      if (edge_from_coordinate == edge_to_coordinate)
        return false;
      const exact_geometry::Point edge_from(
        edge_from_coordinate.first, edge_from_coordinate.second);
      const exact_geometry::Point edge_to(
        edge_to_coordinate.first, edge_to_coordinate.second);
      const auto relation = classifyExactSegments(
        previous, next, edge_from, edge_to);
      ...
      if (allowed_contact &&
          relation.relation == ExactSegmentRelation::endpoint_touch &&
...
PolymapCreateResult Polymap::create(
  const GridMap& grid_map, int start_x, int start_y,
  int goal_x, int goal_y, const PlanningLimits& limits)
{
  return create(
    grid_map, start_x, start_y, goal_x, goal_y, StopToken{}, limits);
}
  ...
  const PlanningLimits& limits)
{
  ...
  if (!validateMapResourceBudget(
      static_cast<size_t>(grid_map.width),
      static_cast<size_t>(grid_map.height), grid_map.data.size(),
      limits, estimate, result.error))
  {
    return result;
  }
  (void)estimate;
  return finishCreation(Polymap(
    grid_map, start_x, start_y, goal_x, goal_y, stop_token));
}
...
PolymapCreateResult Polymap::finishCreation(Polymap&& candidate)
{
  ...
  if (!candidate.solution_exist_ || !candidate.cdt_ready_)
  {
    ...
    result.error = candidate.construction_error_.empty() ?
      "Map geometry construction failed" :
...
Polymap::Polymap(const GridMap& grid_map, int start_x, int start_y,
  int goal_x, int goal_y, const StopToken& stop_token)
  : xsize_(0), ysize_(0)
{
  const auto stop_construction = [&]() {
  ...
  size_t cell_count = 0;
  if (!prepareGridStorage(
      grid_map, xsize_, ysize_, cell_count, construction_error_))
  {
    return;
  }
  data_ = grid_map.data;
  ...
  if (start_x < 0 || start_y < 0 || goal_x < 0 || goal_y < 0 ||
      start_x >= xsize_ || start_y >= ysize_ ||
      goal_x >= xsize_ || goal_y >= ysize_)
  {
    construction_error_ = "Start or goal grid cell is outside map bounds";
    return;
  }
  const size_t start_index = static_cast<size_t>(start_x) +
    static_cast<size_t>(start_y) * static_cast<size_t>(xsize_);
  const size_t goal_index = static_cast<size_t>(goal_x) +
    static_cast<size_t>(goal_y) * static_cast<size_t>(xsize_);
  ...
  const OperationStatus obstacle_status = getPolyObstacles(
    start_x, start_y, goal_x, goal_y, stop_token);
  ...
  status = validateObstacleTopology(
    construction_error_, false, stop_token);
  if (status == OperationStatus::stopped)
  {
    stop_construction();
    return;
  ...
  status = validateObstacleTopology(
    construction_error_, true, stop_token);
  if (status == OperationStatus::stopped)
  {
    stop_construction();
    return;
  ...
  status = constructCGALRelated(
    &validateCDTWithCGAL, construction_error_, stop_token);
  if (status == OperationStatus::stopped)
  {
    stop_construction();
    return;
...
PolymapCreateResult Polymap::create(
  ...
  const Point2d& goal_position, const PlanningLimits& limits)
{
  return create(
    grid_map, start_x, start_y, goal_x, goal_y,
    start_position, goal_position, StopToken{}, limits);
}
  ...
  const PlanningLimits& limits)
{
  ...
  return finishCreation(Polymap(
    grid_map, start_x, start_y, goal_x, goal_y,
    start_position, goal_position, stop_token));
}
  ...
  : xsize_(0), ysize_(0)
{
  ...
  if (!std::isfinite(start_position.first) ||
      !std::isfinite(start_position.second) ||
      !std::isfinite(goal_position.first) ||
      !std::isfinite(goal_position.second))
  {
  ...
  const size_t goal_index = static_cast<size_t>(goal_x) +
    static_cast<size_t>(goal_y) * static_cast<size_t>(xsize_);
  if (start_index >= data_.size() || goal_index >= data_.size() ||
      data_[start_index] != 0 || data_[goal_index] != 0)
  {
  ...
  {
    status = validateFreeSpaceInterior(
      start_position, stop_token, &endpoint_error);
    if (status == OperationStatus::stopped)
    {
      stop_construction();
      return;
    ...
    status = validateFreeSpaceInterior(
      goal_position, stop_token, &endpoint_error);
    if (status == OperationStatus::stopped)
    {
      stop_construction();
      return;
...
OperationStatus Polymap::registerVertices(
...
bool Polymap::registerVerticesImpl(
  std::string& error, const StopToken& stop_token)
{
  error.clear();
  if (stop_token.poll())
  ...
  for (size_t i = 0; i < obs_.size(); ++i)
  {
    ...
    for (size_t j = 0; j < obs_[i].ordered_vertices_.size(); ++j)
    {
      ...
      {
        error = "Unsupported obstacle topology: obstacle " +
          std::to_string(i) + " vertex " + std::to_string(j) + " at (" +
      ...
      const size_t index = static_cast<size_t>(y) *
        static_cast<size_t>(xsize_) + static_cast<size_t>(x);
      if (candidate_obstacles[index] >= 0)
      {
        error = "Unsupported obstacle topology: obstacle " +
          std::to_string(i) + " vertex " + std::to_string(j) +
          " would overwrite obstacle " +
...
bool Polymap::simplifyPolyObstaclesImpl(
  const Point2d& start, const Point2d& goal,
  const StopToken& stop_token)
{
  for (auto iter = obs_.begin(); iter != obs_.end(); ++iter)
  {
    ...
    while (1)
    {
      ...
      const CGAL::Orientation turn =
        CGAL::orientation(previous_point, current_point, next_point);
      ...
      if (turn == CGAL::COLLINEAR)
      {
        ...
        simplifable = exact_geometry::isRemovableCollinearMiddle(
          ...
          if (simplifable)
          {
            for (auto iter2 = obs_.begin(); iter2 != obs_.end(); ++iter2)
            {
              ...
              for (auto iter3 = iter2->ordered_vertices_.begin();
                iter3 != iter2->ordered_vertices_.end(); ++iter3)
              {
                if (stop_token.poll())
                  return false;
                testx = iter3->first;
...
{
  if (facet_idx < 0 || facet_idx >= static_cast<int>(facets_.size())) return false;
  ...
  for (size_t oi = 1; oi < obs_.size(); ++oi)
  {
    ...
    for (int k = 0, j = n - 1; k < n; j = k++)
    {
      if (((v[k].second > cy) != (v[j].second > cy)) &&
          (cx < (v[j].first - v[k].first) * (cy - v[k].second) / (v[j].second - v[k].second + 1e-15) + v[k].first))
        inside = !inside;
    }
